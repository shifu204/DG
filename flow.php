...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    $is_gift = isset($_REQUEST['is_gift']) ? intval($_REQUEST['is_gift']) : 0;
    // 解决IE6生成的JSON不能解码的问题
    $_POST['goods'] = str_replace('NaN', 'null', $_POST['goods']);
...
//    }
    ...
    {
        // 更新：添加到购物车
        if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent, $is_gift))
        {
            ...
            $result['cart_info'] = get_cart_info();
        }
        else
        {
...
} elseif($_REQUEST['step'] == "check_login"){
    $direct_shopping = $_SESSION["direct_shopping"];
    $result['error'] = "";
    if(empty($_SESSION['user_id']) && empty($direct_shopping)){
        //需要买家选择直接购物或者登录购物
        $result['error'] = 1;
        $result['need_select'] = 1;
        $result['content'] = $smarty->fetch("library/user/register_login.lbi");             
    }
    die(json_encode($result));
}
elseif ($_REQUEST['step'] == 'checkout')
{
    /*检测买家是否登录或者是否已经选择直接购物*/
//    if(empty($_SESSION['user_id']) && ($_REQUEST['direct_shopping'] != 1 && $_SESSION['direct_shopping']!= 1 )){
//        ecs_header("Location: flow.php?step=cart\n");
//        exit;
//    }
    if(empty($_SESSION['user_id'])){      
        ecs_header("Location: user.php?back_act=cart");
        exit;
    }
    if(isset($_REQUEST['direct_shopping'])){
    ...
    if ($db->getOne($sql) == 0)
    {
        show_message($_LANG['no_goods_in_cart'], '去购物', 'index.php', 'warning');
    }
    ...
    $default_address_id = get_default_address_id($addresses);
    
    if(isset($_SESSION['flow_consignee']) && !empty($_SESSION['flow_consignee'])){
        $consignee = $_SESSION['flow_consignee'];
    } else {
        $consignee = get_default_consignee($default_address_id);
    }
    ...
    if(!empty($shipping_list)){
        foreach($shipping_list as $shipping){
            if($shipping['shipping_code'] == 'sf_express'){
    ...
    if ($total['goods_price'] < 50) {
        ecs_header("Location: flow.php?src=checkout\n");
        exit;
    }
    ...
    foreach ($shipping_list AS $key => $val)
    {
        ...
        $cart_weight_price['weight'], $cart_weight_price['amount'], $cart_weigorder_max_integralht_price['number']);
    ...
    $smarty->assign('user_discount', $_SESSION['discount'] * 10);
...
elseif ($_REQUEST['step'] == 'select_shipping')
{
    ...
    /* 获得收货人信息 */
    if($_REQUEST['address_id'] == "new_address"){
        $_REQUEST['address_id'] = 0;
    }
    $address_id = empty($_REQUEST['address_id'])?0:$_REQUEST['address_id'];
    if(!empty($address_id)){
        $consignee = get_consignee_by_address_id($address_id);
    } else {
        $consignee = $_SESSION['flow_consignee'];
    }
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    $order = array(
        ...
        'is_mobile'       => !empty($mobile_templete) ? 1 : 0
        );
...
//    }
   ...
   /* 判断是不是实体商品 */
    ...
    if ($order['shipping_id'] > 0)
    {
        ...
        show_message(sprintf($_LANG['shipping_not_select'], false));
    }
    ...
    if ($order['pay_id'] > 0)
    {
        ...
        $order['pay_logo'] = $payment['pay_logo'];
    } else {
        show_message(sprintf($_LANG['payment_not_select'], false));
    }
    ...
    /*记录页面浏览轨迹，下订单*/
    //trace::trace_browse(TRACE_ORDER_SUBMIT, $order['order_sn']);
    /* 清空购物车 */
    ...
    if ($order['order_amount'] > 0)
    {
        ...
        $pay_online = get_pay_online($order['order_sn']);
...
elseif ($_REQUEST['step'] == 'pay_success') {
    ...
    $smarty->assign('order',$order);
    $result['err'] = 0;
    $respond = pay_success_respond();
    if(!empty($respond)) {
        $result['redict_url'] = $respond['redict_url'];
    }
    $result['content'] = $smarty->fetch('library/flow/flow_pay_done.lbi');
    die(json_encode($result));
}
...
elseif ($_REQUEST['step']== 'ajax_update_cart'){
    ...
    $checkout = isset($_REQUEST['from_checkout']) ? 1 : 0;
    $json = new JSON();
    $result = array('error' => 0, 'message'=> '');
    if (isset($_POST['rec_id']) &&isset($_POST['goods_number'])){
        $rec_id = $_POST['rec_id'];
        $goods_number = $_POST['goods_number'];
        $goods_number = intval(make_semiangle($goods_number));
        if ($goods_number < 1 || !is_numeric($rec_id)){
            $result['error'] = 1;
            $result['message'] = $GLOBALS['_LANG']['goods_number_not_int'];//没有输入正确的商品数量      
            die($json->encode($result));
        }

        //查询：
        $sql = 'SELECT c.`goods_id`, c.`brand_id`, c.`goods_attr_id`,'
             . ' c.`product_id`, c.`extension_code`,'
             . ' g.is_buy_five, g.is_buy_seven, b.`is_preferential_five`, b.`is_preferential_seven` '
             . ' FROM ' . $GLOBALS['ecs']->table('cart') . ' AS c'
             . ' LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b'
             . ' ON b.brand_id = c.brand_id '
             . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g'
             . ' ON g.goods_id = c.goods_id '
             . " WHERE c.rec_id = '$rec_id' AND c.session_id='" . SESS_ID . "'";
        ...
        if(!$goods){
            $result['error'] = 1;
            // 您的购物车中没有商品
            $result['message'] = $GLOBALS['_LANG']['no_goods_in_cart'];
            die($json->encode($result));
	}
        ...
        $offers_accessories_res =$GLOBALS['db']->query($sql);$sql = '';
		...
		if ($goods_number > 0){
            ...
            if ($goods['extension_code'] =='package_buy'){
                //更新购物车中的商品数量
                $sql = "UPDATE ".$GLOBALS['ecs']->table('cart')." SET goods_number= '$goods_number' WHERE rec_id='$rec_id' AND session_id='" . SESS_ID . "'";
            }else{/* 处理普通商品或非优惠的配件*/
                $attr_id    = empty($goods['goods_attr_id']) ? array(): explode(',', $goods['goods_attr_id']);
                $goods_price =get_final_price($goods['goods_id'], $goods_number, true, $attr_id);

                //更新购物车中的商品数量
                $sql = "UPDATE ".$GLOBALS['ecs']->table('cart')
                     . " SET goods_number= '$goods_number', goods_price = '$goods_price'"
            ...
            $sql = "DELETE FROM ".$GLOBALS['ecs']->table('cart')." WHERE rec_id='$rec_id' AND session_id='" .SESS_ID. "'";
        }
        if($sql)$GLOBALS['db']->query($sql);
        ...
        $sql = 'SELECT SUM(`goods_number`) AS count FROM ' . $GLOBALS['ecs']->table('cart')
             . ' WHERE session_id = \'' . SESS_ID . '\' AND `brand_id` = ' . $goods['brand_id']
             . ' AND is_gift = 0';
        $result['brand_id']             = $goods['brand_id'];
        $result['brand_goods_count']    = $GLOBALS['db']->getOne($sql);

        $result['target_qty'] = getTargetQuantity($goods['goods_id']);
        if ($result['target_qty'] > 0) {
            $result['balance_qty']      = $result['target_qty'] - $result['brand_goods_count'];
            if ($result['balance_qty'] > 0) {
                $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart')
                     . " WHERE session_id = '" . SESS_ID . "' AND is_gift <> 0"
                     . " AND `brand_id` = " . $goods['brand_id'];
                FB::log($sql);
                $GLOBALS['db']->query($sql);
            }
        }
        ...
        $sql = 'SELECT SUM(`goods_number`) AS count FROM ' . $GLOBALS['ecs']->table('cart')
             . ' WHERE session_id = \'' . SESS_ID . '\' AND `brand_id` = ' . $goods['brand_id']
             . ' AND is_gift = 1';
        $result['gift_number']    = $GLOBALS['db']->getOne($sql);
        if($checkout == 1){
            //获取订单费用信息
            $flow_type = isset($_SESSION['flow_type']) ? intval($_SESSION['flow_type']) : CART_GENERAL_GOODS;
            $cart_goods = cart_goods($flow_type);
            $order = flow_order_info();
            $consignee = get_consignee($_SESSION['user_id']);       
            $order_total = order_fee($order, $cart_goods, $consignee);
            $result['total'] = $order_total;
            $smarty->assign('total', $order_total);    
            $smarty->assign('config', $_CFG);
...
//-- 选择赠品
/*------------------------------------------------------ */
elseif ($_REQUEST['step'] == 'select_gift')
{
    $brand_id = intval($_REQUEST['brand_id']);
    $gift_type = intval($_REQUEST['gift_type']);
    if ($gift_type == BUY_FIVE_QTY) {
        $where = ' AND `is_buy_five` = 1';
    } else {
        $where = ' AND `is_buy_seven` = 1';
    }
    $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('goods')
         . ' WHERE `brand_id` = ' . $brand_id . ' AND `is_delete` = 0' . $where;
    FB::log($sql);
    ...
    if ($mobile_templete) {
        ...
        foreach ($gift_list as $key => $value) {
            $html .= '<li><a href="javascript:addToCart(' . $value['goods_id'] . ', \'\', 1);"><img src="' . $value['goods_thumb'] . '" alt="" /></a>';
            $html .= '<p class="goods_name">' . $value['goods_name'] . '</p>';
            $html .= '<p class="goods_price">德贝价：<del>' . $value['shop_price'] . '</del></p>';
            $html .= '<a class="select_gift" href="javascript:addToCart(' . $value['goods_id'] . ', \'\', 1);">选择赠品</a>';
            $html .= '</li>';
        }
        echo $html;
        exit;
...
elseif ($_REQUEST['step'] == 'select_gift_m')
{
    $brand_id = intval($_REQUEST['brand_id']);
    $gift_type = intval($_REQUEST['gift_type']);
    if ($gift_type == BUY_FIVE_QTY) {
    ...
    foreach ($gift_list as $key => $value) {
        $html .= '<li><a href="javascript:addToCart(' . $value['goods_id'] . ', \'\', 1);"><img src="' . $value['goods_thumb'] . '" alt="" /></a>';
        $html .= '<p class="goods_name">' . $value['goods_name'] . '</p>';
        $html .= '<p class="goods_price">极致品质价：<del>' . $value['shop_price'] . '</del></p>';
        $html .= '<a class="select_gift" href="javascript:addToCart(' . $value['goods_id'] . ', \'\', 1);">选择赠品</a>';
        $html .= '</li>';
    }
    echo $html;
    exit;
} else if($_REQUEST['step'] == 'save_consignee'){
    //保存收货人更改信息
    $result = array();
    $consignee_data = $_REQUEST['consignee_data'];
    $selected = isset($_REQUEST['selected'])?intval($_REQUEST['selected']):0;
    $smarty->assign("default_address_id",$selected);
    $user_id = $_SESSION['user_id'];
    if(!empty($consignee_data)){
        //买家登录了，更改数据库
        if(!empty($user_id)){
            foreach ($consignee_data as $ck=>$cv){
                //address_id为空则是新添加的地址
                if(empty($cv['address_id'])){
                    if(!empty($cv['consignee']) && (!empty($cv['mobile']) || !empty($cv['tel'])) && !empty($cv['address'])){
                        //插入数据库
                        $insert['consignee'] = $cv['consignee'];
                        $insert['tel'] = $cv['tel'];
                        $insert['mobile'] = $cv['mobile'];
                        $insert['address'] = $cv['address'];
                        $insert['country'] = 1;
                        $insert['province'] = $cv['province'];
                        $insert['city'] = $cv['city'];
                        if(isset($cv['district']) && $cv['district'] != 'null'){
                            $insert['district'] = $cv['district'];
                        }
                        $insert['user_id'] = $user_id;
                        $insert['id_num'] = '';
                        //$insert['zipcode'] = '0';
                        //$insert['email'] = '';
                        //$insert['sign_building'] = '';
                        //$insert['best_time'] = '';
                        $GLOBALS['cidb']->insert('user_address',$insert);
                        $result['new_id'] = $GLOBALS['cidb']->insert_id();                  
                    }
                } else if($cv['address_id'] == "new_address"){
                    $_SESSION['flow_consignee'] = $cv;
                } else if(intval($cv['address_id']) > 0 ) {
                    //$update['address_id'] = $cv['address_id'];
                    if(!empty($cv['consignee'])){
                        $update['consignee'] = $cv['consignee'];
                    }
                    if(!empty($cv['mobile'])){
                        $update['mobile'] = $cv['mobile'];
                    }
                    if(!empty($cv['tel'])){
                        $update['tel'] = $cv['tel'];
                    }
                    if(!empty($cv['address'])){
                        $update['address'] = $cv['address'];
                    }
                    $update['province'] = $cv['province'];
                    $update['city'] = $cv['city'];
                    if(isset($cv['district']) && $cv['district'] != 'null'){
                        $update['district'] = $cv['district'];
                    }
                    $GLOBALS['cidb']->where('address_id = '.$cv['address_id'])->update('user_address',$update);
                }
            }
        //没有登录
        } else {
            $_SESSION['flow_consignee'] = $consignee_data[0];
        }
        $result['error'] = 0;
    } else {
        //收货人信息为空
    ...
    $smarty->assign("addresses",$addresses);  
    $smarty->assign('default_address_id', $default_address_id);
    if(isset($_REQUEST['from']) && $_REQUEST['from'] == 'my_consignee'){
        $smarty->assign("add_num", count($addresses));
        $result['addresses'] = $smarty->fetch("library/user/my_consignee.lbi");
    } else {
        $result['addresses'] = $smarty->fetch("library/flow/flow_consignee_address.lbi");
    }
    die(json_encode($result));
} else if($_REQUEST['act'] == "delete_address"){
    $address_id = empty($_REQUEST['address_id'])?0:$_REQUEST['address_id'];
    $result = array();
    $result['error'] = '';
    if($address_id > 0){
        $res = $GLOBALS['cidb']->where(array("address_id"=>$address_id,"user_id"=>$_SESSION['user_id']))->limit(1)->delete("user_address");
        if($res){
            $addresses = get_address();
            if($address_id == $user_info['address_id']){
                include_once (ROOT_PATH . 'includes/lib_transaction.php');
                edit_profile(array('user_id'=>$_SESSION['user_id'], 'address_id'=> 0 ));
    ...
    die(json_encode($result));
} else if($_REQUEST['act'] == "set_default_address"){
    $address_id = empty($_REQUEST['address_id'])?0:$_REQUEST['address_id'];
    if($address_id > 0){
        $result = $GLOBALS['cidb']->where(array("user_id"=>$_SESSION['user_id']))->limit(1)->update("users",array("address_id"=>$address_id));
        if($result){
            die(json_encode(1));
        }
    }
    die(json_encode(0));
} else if($_REQUEST['act'] == "get_shipping"){
    /*------------------------------------------------------ */
    //-- 获取配送方式
    /*------------------------------------------------------ */
    $result = array('error' => '', 'content' => '', 'need_insure' => 0);
    ...
    $result['need_insure'] = ($shipping_info['insure'] > 0 && !empty($order['need_insure'])) ? 1 : 0;
    $shipping_list     = get_shipping_list(array("province"=>$consignee['province'],"city"=>$consignee['city'],"district"=>$consignee['district']),$flow_type);
    $smarty->assign('shipping_list', $shipping_list);
    $result['content'] = $smarty->fetch("library/flow/flow_shipping_method.lbi");
    if(!empty($consignee)){
        $_SESSION['flow_consignee'] = $consignee;
    }
    die(json_encode($result));
...
else
{
    ...
    if ($_REQUEST['src'] == 'checkout') {
        $smarty->assign('price_tips', 1);
    }
}
...
$orders = get_orders(array("order_status"=>ORDER_VALID, "no_cod"=>1,'where'=>" AND pay_status <= ".PS_PAYING." AND shipping_status <= ".SS_UNSHIPPED));
...
function get_default_consignee($address_id = ''){
    $result = array();
    $user_id = $_SESSION['user_id'];
    if (isset($_SESSION['flow_consignee']['default']) && !empty($_SESSION['flow_consignee']['default']))
    {
        /* 如果存在session，则直接返回session中的收货人信息 */
        $result = $_SESSION['flow_consignee']['default'];
    }else {
        if($user_id > 0){
            $sql = "SELECT ua.*".
            ...
            if(!empty($address_id)){
                $sql .= " AND ua.address_id = $address_id";
            }
            $result = $GLOBALS['db']->getRow($sql);
            if($result){
                $result['region_name'] = getRegionName(0, $result['country'], $result['province'], $result['city'], $result['district']);
            }
        }
    }
    return $result;
...
function get_consignee_by_address_id($address_id = 0){
    if(!empty($address_id)){
        $result = $GLOBALS['cidb']->where("user_address.address_id = ".$address_id)->get("user_address")->row_array();
        return $result;
    } else {
        return false;
    }
...
function get_shipping_list($region,$flow_type = CART_GENERAL_GOODS){
    //$region            = array($consignee['country'], $consignee['province'], $consignee['city'], $consignee['district']);
    $region['country'] = 1;
    $order['shipping_id'] = intval($_REQUEST['shipping']);
    $flow_type = isset($_SESSION['flow_type']) ? intval($_SESSION['flow_type']) : CART_GENERAL_GOODS;
...
function get_address(){ 
    $addresses = array(); 
    //现在修改为不做跳转，直接在订单信息页面上修改用户的信息
    $addresses = address_list($_SESSION['user_id']);
    if(!empty($consignee)){
        if(empty($consignee['address_id']) || $consignee['address_id']=="new_address"){
            $consignee['country'] = 1;
            $addresses[] = $consignee;
        }
    }
    foreach($addresses as $ak=>$address){
        ...
        if(empty($address['address_id'])){
            $addresses[$ak]['address_id'] = 'new_address';
        }
    }
    return $addresses;
}

function get_default_address_id($addresses){
    $user_info = $GLOBALS['user_info'];
    $flow_consignee = $_SESSION['flow_consignee'];
    if(!empty($flow_consignee)){
        $default_address_id = $flow_consignee['address_id'];
    } else {
        $default_address_id = empty($user_info['address_id'])? 0 : $user_info['address_id'];
    }
    //设置默认选中的地址
