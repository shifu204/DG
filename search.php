...
if ($_REQUEST['act'] == 'advanced_search')
{
    ...
    $smarty->assign("search_keywords",  explode(',',$_CFG['search_keywords']));
...
//        }
        else
        {
            /* 检查关键字中是否有空格，如果存在就是或 */
            //对关键字进行分词
            $split_keywords = splitword($_REQUEST['keywords']);
            if($split_keywords){
                if(count($split_keywords) >= 1){
                    $_SESSION['search_keywords_split'] = $split_keywords;
                    $arr = $split_keywords;
                    $operator = " OR ";
                }
                else
    ...
    /* 查询商品 */
    $sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, g.is_buy_six, g.is_buy_nine, g.is_total_four, ".
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                "g.promote_price, g.promote_start_date, g.promote_end_date, g.goods_thumb, g.goods_img, g.goods_brief, g.goods_type, goods_same_id ".
    ...
    $arr = array();
    while(list($key, $word) = each($_SESSION['search_keywords_split'])){
        $keyword_replace[] = "<span style='color:red'>".$word."</span>";
    }
    while ($row = $db->FetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['final_price']    = get_final_price($row['goods_id']);
        $arr[$row['goods_id']]['formated_final_price']    = price_format($arr[$row['goods_id']]['final_price'],false);
        $arr[$row['goods_id']]['promote_price'] = ($promote_price > 0) ? price_format($promote_price) : '';
        $arr[$row['goods_id']]['goods_brief']   = $row['goods_brief'];
        $arr[$row['goods_id']]['goods_thumb']   = get_image_path($row['goods_id'], $row['goods_thumb'], true);
        $arr[$row['goods_id']]['goods_img']     = get_image_path($row['goods_id'], $row['goods_img']);
        $arr[$row['goods_id']]['url']           = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
        $arr[$row['goods_id']]['is_buy_six']    = $row['is_buy_six'];
        $arr[$row['goods_id']]['is_buy_nine']   = $row['is_buy_nine'];
        $arr[$row['goods_id']]['is_total_four'] = $row['is_total_four'];
        
        //对goods_name进行关键字格式化               
        $arr[$row['goods_id']]['goods_name']    = str_replace($_SESSION['search_keywords_split'], $keyword_replace, $row['goods_name']);
        //图片为空
