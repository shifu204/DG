...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
            ...
            global $mobile_templete;
            if (empty($mobile_templete)) {
                ...
                $str.='<a href="'.$goods['url'].'" target="_blank"><ul class="clearfix"><li class="goodsimg"><img src="'.$goods['goods_thumb'].'" alt="'.$goods['goods_name'].'" class="B_blue" /></li><li>'.$goods['short_name'].'<br />'.$GLOBALS['_LANG']['shop_price'].'<font class="f1">'.$goods['shop_price'].'</font><br /></li></ul></a>';
            }
        }
...
function insert_member_info_s()
{
    ...
    $output = $GLOBALS['smarty']->fetch('library/member_info_s.lbi');
 ...
 * @return  string
 */
function insert_goods_comments($arr)
{
    $need_cache = $GLOBALS['smarty']->caching;
    $need_compile = $GLOBALS['smarty']->force_compile;
    ...
    $GLOBALS['smarty']->assign('comment_type', $arr['type']);
    $GLOBALS['smarty']->assign('id',           $arr['id']);
    //如果该商品存在相同商品的ID，则调用相同商品的评论
    if(!empty($arr['goods_same_id'])){
        $cmt = assign_comment($arr['goods_same_id'],          $arr['type']);
    } else {
        $cmt = assign_comment($arr['id'],          $arr['type']);
    }
    $GLOBALS['smarty']->assign('comments',     $cmt['comments']);
    ...
    $val = $GLOBALS['smarty']->fetch('library/goods/comments_list.lbi');
...
{
    $val = $GLOBALS['smarty']->fetch('library/order_total.lbi');
    return $val;
}
