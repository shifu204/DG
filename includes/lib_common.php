 ...
 * 获取帐户变动记录
 */
function get_log_account($user_id = 0, $change_type = 0){
    $sql = " SELECT * FROM " . $GLOBALS['ecs']->table('account_log') . " WHERE 1";
    if($user_id > 0 ){
...
{
    // 参加买五送一后的价格（一般是原价）
    $after_preferential_price = getAfterPreferentialPrice($goods_id);
    FB::Error($after_preferential_price);
    if ($after_preferential_price) {
        $final_price = $after_preferential_price;
    } else {
...
// 获取目录数量
function getTargetQuantity($goods_id) {
    $sql = 'SELECT g.is_buy_five, g.is_buy_seven,'
	     . ' b.is_preferential_five, b.is_preferential_seven'
         . ' FROM' . $GLOBALS['ecs']->table('goods') . ' AS g'
         . ' LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b'
         . ' ON b.brand_id = g.brand_id WHERE g.goods_id = ' . $goods_id;
    $preferential = $GLOBALS['db']->getRow($sql);
    $target_quantity = 0;
    if ($preferential['is_preferential_five'] && $preferential['is_buy_five']) {
        $target_quantity = BUY_FIVE_QTY;
    } else if ($preferential['is_preferential_seven'] && $preferential['is_buy_seven']) {
        $target_quantity = BUY_SEVEN_QTY;
    }
    return $target_quantity;
}

function getAfterPreferentialPrice($goods_id) {
    $after_preferential_price = 0;
    $target_qty = getTargetQuantity($goods_id);
    FB::Error(BUY_FIVE_QTY);
    if ($target_qty == BUY_FIVE_QTY) {
        $sql = 'SELECT `brand_id` FROM ' . $GLOBALS['ecs']->table('goods')
             . ' WHERE `goods_id` = ' . $goods_id;
        ...
        $sql = 'SELECT SUM(c.`goods_number`)'
             ...
             . ' WHERE b.`is_preferential_five` = 1 AND g.`brand_id` = ' . $brand_id
             . ' AND g.`is_buy_five` = 1 AND c.session_id = \'' . SESS_ID . '\'';

        // 先检查该产品对应的品牌中，用户有没有领取过赠品
        $sql1 = $sql . ' AND c.`is_gift` = 1';
        $count_brand_goods_for_gift = $GLOBALS['db']->getOne($sql1);
        FB::log('$count_brand_goods_for_gift');
        FB::Error($count_brand_goods_for_gift);

        // 再检查该产品对应的品牌中，用户购买了多少
        //$sql2 = $sql . ' AND c.`is_gift` = 0';
        //$count_brand_goods_for_preferential = $GLOBALS['db']->getOne($sql2);
        //FB::log('$count_brand_goods_for_preferential');
        //FB::Error($count_brand_goods_for_preferential);

        if ($count_brand_goods_for_gift > 0) {
            $sql = 'SELECT `shop_price` FROM ' . $GLOBALS['ecs']->table('goods')
                 . ' WHERE `goods_id` = ' . $goods_id;
            $after_preferential_price = $GLOBALS['db']->getOne($sql);
        }
    }
    return $after_preferential_price;
}

function get_user_name($user_info = array()){
    if(empty($user_info)){
