 ...
 * @return  array
 */
function get_categories_tree($cat_id = 0, $is_show = 1)
{
    if ($cat_id > 0)
    {
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
    */
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('category') . " WHERE parent_id = '$parent_id' AND is_show = $is_show ";
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT cat_id,cat_name ,parent_id,is_show,cat_tag, cat_ico ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
                "WHERE parent_id = '$parent_id' AND is_show = $is_show ORDER BY sort_order ASC, cat_id ASC";

        $res = $GLOBALS['db']->getAll($sql);

        foreach ($res AS $row)
        {
            if ($row['is_show'] == $is_show)
            {
                $cat_arr[$row['cat_id']]['id']   = $row['cat_id'];
                $cat_arr[$row['cat_id']]['name'] = $row['cat_name'];
                $cat_arr[$row['cat_id']]['url']  = build_uri('category', array('cid' => $row['cat_id']), $row['cat_name']);
                $cat_arr[$row['cat_id']]['cat_tag'] = $row['cat_tag'];
                $cat_arr[$row['cat_id']]['cat_ico'] = $row['cat_ico'];
...
function get_catlist($cat_id = 0, $is_show = 1)
{
    $parent_id = $cat_id;
...
{
    $three_arr = array();
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('category') . " WHERE parent_id = '$tree_id' AND is_show = $is_show ";
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0)
    {
        $child_sql = 'SELECT cat_id, cat_name, parent_id, is_show, cat_tag, cat_ico ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
                "WHERE parent_id = '$tree_id' AND is_show = $is_show ORDER BY sort_order ASC, cat_id ASC";
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'] == $is_show)
            {
               ...
               $three_arr[$row['cat_id']]['cat_tag'] = $row['cat_tag'];
               $three_arr[$row['cat_id']]['cat_ico'] = $row['cat_ico'];

               if (isset($row['cat_id']) != NULL)
               {
                   $three_arr[$row['cat_id']]['cat_id'] = get_child_tree($row['cat_id'], $is_show);
               }
            }
        }
    }
 ...
 * @return  array
 */
function get_recommend_goods($type = '', $cats = '', $num = 3)
{
...
function get_promote_goods($cats = '')
{
    ...
    foreach ($result AS $idx => $row)
    {
        ...
        $goods[$idx]['goods_img']    = get_image_path($row['goods_id'], $row['goods_img']);
        $goods[$idx]['url']          = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
		$goods[$idx]['promote_end_date'] = $row['promote_end_date'];
		//促销商品倒计时开始
		$time = gmtime();
		if ($time >= $row['promote_start_date']&& $time <=$row['promote_end_date']){
                        //@hero
			//$goods[$idx]['gmt_end_time'] = local_date('M d, Y H:i:s',$row['promote_end_date']);
			$goods[$idx]['gmt_end_time'] = $row['promote_end_date'] + 28800;
		}else{
...
{
    $time = gmtime();
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, b.is_preferential_five, b.is_preferential_seven, m.type_money AS bonus_money, ' .
    ...
    $goods_same_id = getGoodsSameId($goods_id);
    $same_goods_single_price = array();
    if ($goods_same_id) {
        // 如果当前id 不等于 goods_same_id，就调用goods_same_id 的描述
        if ($goods_same_id != $goods_id) {
            $sql = 'SELECT goods_desc, original_img FROM ' . $GLOBALS['ecs']->table('goods') . ' WHERE goods_id = ' . $goods_same_id;
            $row_s = $GLOBALS['db']->getRow($sql);
            $row['goods_desc'] .= $row_s['goods_desc'];
            if(empty($row['original_img'])){
                $row['original_img'] = $row_s['original_img'];
            }
        }

        $sql = 'SELECT goods_id, goods_short_name, goods_name, shop_price, goods_img, is_buy_six, is_buy_nine, is_total_four, is_total_two, goods_same_id  FROM ' . $GLOBALS['ecs']->table('goods')
             . ' WHERE goods_same_id = ' . $goods_same_id . ' AND is_delete = 0 AND is_on_sale = 1' .' AND goods_id != ' .$goods_id. ' ORDER BY sort_order ASC ';
        $same_goods = $GLOBALS['db']->getAll($sql);
        
        foreach($same_goods as $key => $goods) {
            $same_goods[$key]['single_price'] = price_format($goods['shop_price'] / intval($goods['goods_short_name'])) . '/罐';
            $same_goods[$key]['url'] = build_uri('goods', array('gid'=>$goods['goods_id']), $goods['goods_name']);
            array_push($same_goods_single_price, round($goods['shop_price'] / intval($goods['goods_short_name'])));
            if ($goods['goods_id'] == $goods_id) {
                $same_goods[$key]['is_selected'] = 1;
            }
            if(empty($goods['goods_img'])){
                if(!empty($goods['goods_same_id'])){
                    $sql = "SELECT goods_thumb, goods_img, original_img FROM ".$GLOBALS['ecs']->table('goods')." WHERE goods_id = {$goods['goods_same_id']}";
                    $same = $GLOBALS['db']->getRow($sql);
                    $same_goods[$key]['goods_img'] = $same['goods_img'];
                    $same_goods[$key]['goods_thumb'] = $same['goods_thumb'];
                    $same_goods[$key]['original_img'] = $same['original_img'];
                }
            }
            $same_goods[$key]['formated_shop_price'] = price_format($goods['shop_price'],false);
        }
        $row['same_goods'] = $same_goods;
    }
    /*修正优惠套餐的原价 @author ming*/
    $single_price = round($row['shop_price'] / intval($row['goods_short_name']));
    array_push($same_goods_single_price,$single_price);
    $max_price = max($same_goods_single_price);
    $row['original_price'] =price_format($max_price);
    if($single_price < $max_price ){
        unset( $row['same_goods']);
    }
    if ($row !== false)
    {
        /* 用户评论级别取整 */
        ...
        $row['shop_price_formated'] = price_format($row['shop_price'],false);
        ...
        /*修正商品详情图片，使之延迟加载图片*/
        $row['goods_desc'] = goods_desc_lazyload($row['goods_desc']);
        return $row;
    }
    else
    {
...
function get_goods_properties($goods_id)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
                "g.goods_attr_id, g.attr_value, g.attr_price, gg.thumb_url, gg.img_url, gg.img_original " .
            'FROM ' . $GLOBALS['ecs']->table('goods_attr') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('attribute') . ' AS a ON a.attr_id = g.attr_id ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('goods_gallery') . ' AS gg ON gg.goods_id = g.goods_id and gg.img_desc = g.attr_value' .
...
function get_goods_gallery($goods_id)
{
    ...
    if ($goods_same_id && $goods_same_id != $goods_id) {
        $sql = 'SELECT img_id, img_url, thumb_url, img_desc' .
            ' FROM ' . $GLOBALS['ecs']->table('goods_gallery') .
            " WHERE goods_id = '$goods_same_id' LIMIT " . $GLOBALS['_CFG']['goods_gallery_number'];
        $row_s = $GLOBALS['db']->getAll($sql);
        $row = array_merge($row, $row_s);
    }
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price,g.goods_index_img, g.shop_price AS org_price, g.goods_same_id, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
               'g.promote_price, promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, g.is_buy_five, g.is_buy_seven, b.is_preferential_five, b.is_preferential_seven ' .
            "FROM " . $GLOBALS['ecs']->table('goods') . ' AS g '.
    ...
    foreach ($res AS $idx => $row)
    {
        ...
        $goods[$idx]['short_name']   = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
                                        sub_str($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
        $goods[$idx]['shop_price']   = price_format($row['shop_price'],false);
        $goods[$idx]['thumb']        = get_image_path($row['goods_id'], $row['goods_thumb'], true);
        $goods[$idx]['goods_img']    = get_image_path($row['goods_id'], $row['goods_img']);
        $goods[$idx]['url']          = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
		
	$goods[$idx]['is_buy_five']           = $row['is_buy_five'];
	$goods[$idx]['is_buy_seven']          = $row['is_buy_seven'];
	$goods[$idx]['is_preferential_five']  = $row['is_preferential_five'];
	$goods[$idx]['is_preferential_seven'] = $row['is_preferential_seven'];
        $goods[$idx]['goods_index_img'] = $row['goods_index_img'];
        if(!empty($row['goods_same_id'])){
            $sql = "SELECT goods_thumb, goods_img, original_img, goods_index_img FROM ".$GLOBALS['ecs']->table('goods')." WHERE goods_id = {$row['goods_same_id']}";
            $same = $GLOBALS['db']->getRow($sql);
            $goods[$idx]['goods_img'] = $same['goods_img'];
            $goods[$idx]['goods_thumb'] = $same['goods_thumb'];
            $goods[$idx]['original_img'] = $same['original_img'];
            $goods[$idx]['goods_index_img'] = $same['goods_index_img'];
        }
    }
...
function goods_info($goods_id)
{
    ...
    if (!empty($row))
    {
        ...
        /* 修正图片 */
        $row['goods_img'] = get_image_path($goods_id, $row['goods_img']);
        $row['formated_price'] = price_format($row['shop_price'], true);
    }
...
function getGoodsSameId($goods_id) {
    /* 增加父id商品的图片 */
    $sql = 'SELECT goods_same_id FROM ' . $GLOBALS['ecs']->table('goods') . ' WHERE goods_id = ' . $goods_id;
    $goods_same_id = $GLOBALS['db']->getOne($sql);
    return $goods_same_id;
}

/**
 * 商品详情延时加载，使用正则匹配图片
 * @param string $goods_desc
 */
function goods_desc_lazyload($goods_desc = ''){
    if(!empty($goods_desc)){
        //匹配整个图片标签
        $pattern1 = '/<\s*img\s+[^>]*?src\s*=\s*(\'|\")(.*?)\\1[^>]*?\/?\s*>/i';
        $replace = "<img class=\"lazy\" src=\"/images/loading.gif\" data-original=\"\${2}\"/>";
        $res = preg_replace($pattern1, $replace, $goods_desc);
        return $res;
    }
}
?>
