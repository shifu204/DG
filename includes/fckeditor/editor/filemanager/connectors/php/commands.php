...
function CreateFolder( $resourceType, $currentFolder )
{
    ...
    if ( isset( $_GET['NewFolderName'] ) )
    {
		...
		#-----------------------------
		#解决WIN系统中文文件乱码问题(建目录增加)
		$os = defined( 'PHP_OS' )?PHP_OS:php_uname() ;		
		if ( strtoupper( substr( $os, 0, 3 ) ) === 'WIN' || FindBadUtf8( $sNewFolderName ) ){
...
function FileUpload( $resourceType, $currentFolder, $sCommand )
{
    ...
    if ( isset( $_FILES['NewFile'] ) && !is_null( $_FILES['NewFile']['tmp_name'] ) )
    {
		...
		#-----------------------------
		#解决WIN系统中文文件乱码问题(建文件增加)
		if(!preg_match("/^[\w.]+$/i", $sFileName)){
			$os = defined( 'PHP_OS' )?PHP_OS:php_uname() ;		
			if ( strtoupper( substr( $os, 0, 3 ) ) === 'WIN' || FindBadUtf8( $sFileName ) ){
				$sFileName = iconv("utf-8","gbk",$sFileName);
			}else{
				$rand_str = str_pad(mt_rand(1, 99999), 5, '0', STR_PAD_LEFT) ; //五位随机数
				$sFileName = time() . $rand_str . '.' . strtolower(array_pop(explode('.', $oFile['name'])));
			}
		}
		#解决WIN系统中文文件乱码问题
		#-----------------------------
