...
// @hero
function check_user_exits($user_name){
...
function getUserEmail($user_id) {
    $sql = 'SELECT `email` FROM ' . $GLOBALS['ecs']->table('users')
         . ' WHERE user_id = ' . $_SESSION[user_id];
...
function check_third_user_exists($token, $from = 'qq'){
    $access_token = $from.'_'.$token;
    $sql = "SELECT u.user_id FROM ". $GLOBALS['ecs']->table('users_third_info') . " AS uti " . 
            " RIGHT JOIN " . $GLOBALS['ecs']->table('users') . " AS u ON u.user_id = uti.user_id"
...
function get_user_info($id=0)
{
    ...
    $user['username']    = $user['nickname'] ? $user['nickname'] : $user['user_name'];
 ...
 * @return  array
 */
function get_shop_help($all = false)
{
    $is_show = 1;
    if($all){
        $is_show = ' is_show = 1 AND is_show = 0 ';
    } else {
        $is_show = 'is_show = 1 ';
    }
    ...
    $sql = 'SELECT c.cat_id, c.cat_name, c.sort_order, a.article_id, a.title, a.file_url, a.open_type ' .
            ...
            'ON a.cat_id = c.cat_id WHERE c.cat_type = 5 AND a.is_open = 1 AND '.$is_show .
 ...
 * @return  array       $pager
 */
function get_pager($url, $param, $record_count, $page = 1, $size = 10,$jump='')
{
    $size = intval($size);
    if ($size < 1)
    {
    ...
    else
    {
        ...
        $pager['page_first'] = ($page - $_offset > 1 && $_pagenum < $page_count) ? $url_format . 1 .$jump: '';
        $pager['page_prev']  = ($page > 1) ? $url_format . $page_prev . $jump: '';
        $pager['page_next']  = ($page < $page_count) ? $url_format . $page_next . $jump: '';
        $pager['page_last']  = ($_to < $page_count) ? $url_format . $page_count . $jump: '';
        $pager['page_kbd']  = ($_pagenum < $page_count) ? true : false;
        $pager['page_number'] = array();
        for ($i=$_from;$i<=$_to;++$i)
        {
            $pager['page_number'][$i] = $url_format . $i .$jump;
        }
    }
    $pager['search'] = $param;
...
function assign_comment($id, $type, $page = 1)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if(preg_match($search,$row['user_name'])) {
            $row['user_name'] = substr($row['user_name'], 0, 7 )."****";
        }
        ...
        $arr[$row['comment_id']]['lovenum']  = $row['lovenum'];
        $arr[$row['comment_id']]['title']  = $row['title'];
        if(!empty($row['token'])){
            //获取评论图片
            $sql = "SELECT * FROM ".$GLOBALS['ecs']->table('shaidan_img')." WHERE token = '". $row['token']."'";
            $imgs = $GLOBALS['db']->getAll($sql);
            if(!empty($imgs)){
                $arr[$row['comment_id']]['imgs'] = $imgs;
            }
        }
        /*获取发表评论的会员信息*/
        include_once ROOT_PATH.'includes/lib_transaction.php';
        $user_info = get_profile($row['user_id']);
        $arr[$row['comment_id']]['user_rank'] = $user_info['rank_level'];
    }
    ...
    if ($ids)
    {
        ...
        $where = ' AND user_id > 0 ';
        $res = $GLOBALS['db']->query($sql.$where);
        while ($row = $GLOBALS['db']->fetch_array($res))
        {
            $arr[$row['parent_id']]['re_users'][$row['comment_id']]['re_content']  = nl2br(str_replace('\n', '<br />', htmlspecialchars($row['content'])));
            $arr[$row['parent_id']]['re_users'][$row['comment_id']]['re_add_time'] = local_date($GLOBALS['_CFG']['time_format'], $row['add_time']);
            $arr[$row['parent_id']]['re_users'][$row['comment_id']]['re_email']    = $row['email'];
            $arr[$row['parent_id']]['re_users'][$row['comment_id']]['re_username'] = $row['user_name'];
        }
        //管理员回复
        $where = ' AND user_id = 0 ';
        $res1 = $GLOBALS['db']->query($sql.$where);
        while($row1 = $GLOBALS['db']->fetch_array($res1))
        {
            $arr[$row1['parent_id']]['re_admin']['re_content']  = nl2br(str_replace('\n', '<br />', htmlspecialchars($row1['content'])));
            $arr[$row1['parent_id']]['re_admin']['re_add_time']  = local_date($GLOBALS['_CFG']['time_format'], $row1['add_time']);
            $arr[$row1['parent_id']]['re_admin']['re_email']  = $row1['email'];
            $arr[$row1['parent_id']]['re_admin']['re_username']  = $row1['user_name'];
        }
    }
    /* 分页样式 */
    //$pager['styleid'] = isset($GLOBALS['_CFG']['page_style'])? intval($GLOBALS['_CFG']['page_style']) : 0;
...
function assign_template($ctype = '', $catlist = array())
{
    ...
    $wx_url = urlencode("http://www.deebei.net/api/weixin/weixin.php?act=web_login");
    $smarty->assign('wx_login_url',"https://open.weixin.qq.com/connect/qrconnect?appid=wx01af39a93a1f2179&redirect_uri=$wx_url&response_type=code&scope=snsapi_login&state=STATE#wechat_redirect");   //微信扫描登录地址
    ...
    $smarty->assign('all_catlist', get_allcat_nav());
...
/*获取所有分类*/
function get_allcat_nav(){
    $cat_list = get_categories_tree();
    if(!empty($cat_list)){
        foreach($cat_list as $key => $top_cat){
            /*获取该分类下的扩展分类*/
            $cat_tag = $top_cat['cat_tag'];
            $cat_list[$key]['filter'] = get_child_tree($top_cat['id'], 0);     
            /*获取该分类下的品牌*/
            $cat_list[$key]['brands'] = get_catetory_brands($top_cat['id']);
        }
    }
    return $cat_list;
}
