...
{
    $sql = 'SELECT pay_id, pay_code, pay_name, pay_fee, pay_desc, pay_config, is_cod, pay_logo' .
            ' FROM ' . $GLOBALS['ecs']->table('payment') .
            ' WHERE enabled = 1 ';
    if (!$support_cod)
    {
...
function order_info($order_id, $order_sn = '')
{
    ...
    $should_pay = ",(" . order_should_pay_field() . ") AS should_pay ";
    $order_id = intval($order_id);
    if ($order_id > 0)
    {
        $sql = "SELECT *, " . $total_fee . $should_pay . " FROM " . $GLOBALS['ecs']->table('order_info') .
    ...
    if ($order)
    {
        ...
        $order['express_no'] = explode('<br>', $order['invoice_no']);
        $order['formated_should_pay'] = price_format(abs($order['should_pay']), false);
        $order['formated_discount_amount'] = price_format($order['total_fee'] - $order['should_pay'], false);
    }
 ...
 * @return  boolean
 */
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0, $is_gift = 0)
{
    $GLOBALS['err']->clean();
    $_parent_id = $parent;

    /* 取得商品信息 */
    $sql = "SELECT g.goods_name, g.goods_sn, g.brand_id, g.is_on_sale, g.is_real, ".
    ...
    $target_quantity = getTargetQuantity($goods_id);

    // 检查是否满足赠送赠品的条件
    if ($is_gift == 1) {
        // 如果购买该品牌数量不足目标数，则返回
        $sql = "SELECT SUM(goods_number) AS number_total FROM " .$GLOBALS['ecs']->table('cart').
               " WHERE session_id = '" .SESS_ID. "' AND brand_id =" .
               " (SELECT `brand_id` FROM " . $GLOBALS['ecs']->table('goods') . " WHERE `goods_id` = " . $goods_id . ") ".
               " AND parent_id = 0 AND goods_attr = '" .get_goods_attr_info($spec). "' " .
               " AND extension_code <> 'package_buy' " .
               " AND rec_type = 'CART_GENERAL_GOODS'" .
               " AND is_gift = 0";
        FB::log($sql);
        $number_total = $GLOBALS['db']->getOne($sql);
        // 如果小于目标数，返回
        FB::Warn($target_quantity);
        if ($number_total < $target_quantity) return;
        ...
        $sql = "SELECT SUM(goods_number) AS number_total FROM " .$GLOBALS['ecs']->table('cart').
               ...
               " AND rec_type = 'CART_GENERAL_GOODS'" .
               " AND is_gift = 1";
        $number_gift = $GLOBALS['db']->getOne($sql);
        if ($number_gift > 0) return;
    }
    ...
    if ($num > 0)
    {
        ...
        {
            $goods_price = get_final_price($goods_id, $num, true, $spec);
            $parent['goods_price']  = ($is_gift == 1) ? 0 : max($goods_price, 0);
            $parent['goods_number'] = $num;
            $parent['parent_id']    = 0;
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('cart'), $parent, 'INSERT');

            // 将价格调整回原价
            if ($is_gift == 1 && $target_quantity == BUY_FIVE_QTY) {
                $sql = 'SELECT `brand_id` FROM ' . $GLOBALS['ecs']->table('cart') . ' WHERE `goods_id` = ' . $goods_id . ' LIMIT 1';
                $brand_id = $GLOBALS['db']->getOne($sql);
                $sql = 'UPDATE ' . $GLOBALS['ecs']->table('cart') . ' AS c,'
                     . $GLOBALS['ecs']->table('goods') . ' AS g'
                     . ' SET c.`goods_price` = g.`shop_price`'
                     . ' WHERE c.`goods_id` = g.`goods_id` AND c.`is_gift` = 0 AND g.`brand_id` = ' .$brand_id . ' AND session_id = \'' . SESS_ID . '\'';
                $GLOBALS['db']->query($sql);
            }
        }
    }
...
function address_list($user_id)
{
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('user_address') .
            " WHERE user_id = '$user_id' ORDER BY address_id DESC";
...
function get_cart_goods()
{
    ...
    /* 循环、统计 */
    $sql = "SELECT c.*, IF(parent_id, parent_id, c.goods_id) AS pid "
         . ", b.brand_id, b.brand_name, b.is_preferential_five, b.is_preferential_seven"
         . ", g.shop_price, g.is_buy_five, g.is_buy_seven"
         . " FROM " . $GLOBALS['ecs']->table('cart') . " AS c "
         ...
         . " ORDER BY b.is_preferential_seven, b.is_preferential_five, c.brand_id, c.is_gift, pid, parent_id";
    $res = $GLOBALS['db']->query($sql);
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        // 如果没有创建 $count_brand[品牌ID]，说明是该品牌的第一个商品
        if (!$count_brand[$row['brand_id']]) {
            $count_brand[$row['brand_id']] = $row['goods_number'];
            // 取得每个品牌排序第一的商品的下标
            $brand_first_goods_index[$row['brand_id']] = $i;
            $row['is_first'] = 1;
        } else {
            // 计算单品牌的商品总数
            $count_brand[$row['brand_id']] += $row['goods_number'];
    ...
    FB::log('$brand_first_goods_index');
    FB::info($brand_first_goods_index);
    FB::Warn($goods_list);

    // 在每个品牌排序第一的商品上，记录该品牌商品总数、与目标数的差距、赠品数
    foreach ($brand_first_goods_index as $brand_id => $goods_index) {
       $goods_list[$goods_index]['count'] = $count_brand[$brand_id];

       // 确定买几送一
       $target_quantity = $goods_list[$goods_index]['is_preferential_five'] ? BUY_FIVE_QTY : BUY_SEVEN_QTY;
       $goods_list[$goods_index]['balance_qty'] = $target_quantity - $goods_list[$goods_index]['count'];

       $brand_id = $brand_id ? $brand_id : 0;
       // 检查是否已经有赠品
       $sql = 'SELECT SUM(`goods_number`) AS count FROM ' . $GLOBALS['ecs']->table('cart')
            . ' WHERE session_id = \'' . SESS_ID . '\' AND `brand_id` = ' . $brand_id
            . ' AND is_gift = 1';
       $goods_list[$goods_index]['gift_number'] = $GLOBALS['db']->getOne($sql);
    }
...
function get_consignee($user_id)
{
    ...
    $arr['region_name'] = getRegionName(0, $arr['country'], $arr['province'], $arr['city'], $arr['district']);
    return $arr;
}
...
function check_consignee_info($consignee, $flow_type)
{
    if (exist_real_goods(0, $flow_type))
    {
        ...
        $res = !empty($consignee['consignee']) &&
            ...
            (!empty($consignee['mobile']) || !empty($consignee['tel']));
        if ($res)
        {
...
function order_query_sql($type = 'finished', $alias = '')
{
    ...
    elseif ($type == 'shipped_or_finished')
    {
        return " AND {$alias}order_status " . db_create_in(array(OS_CONFIRMED, OS_SPLITED)) .
               " AND {$alias}shipping_status " . db_create_in(array(SS_SHIPPED, SS_RECEIVED)) . " ";
    }
...
/**
 * 生成计算应付款金额的字段（不扣除已付款金额）
 * @param   string  $alias  order表的别名（包括.例如 o.）
 * @return  string
 */
function order_should_pay_field($alias = ''){
    return order_amount_field($alias) .
            " - {$alias}surplus - {$alias}integral_money" .
            " - {$alias}bonus - {$alias}discount ";
}
...
function getRegionName($order_id = 0, $country = 0, $province = 0, $city = 0, $district = 0) {
    $region_name_merge = '';
    if ($order_id) {
        $sql = "SELECT concat(IFNULL(p.region_name, ''), " . // 删除 IFNULL(c.region_name, ''), '', 
               ...
               "WHERE o.order_id = '$order_id'";
        $region_name_merge = $GLOBALS['db']->getOne($sql);
    } else {
        if ($country > 0) {
            $country  ? $country  : 0;
            $province ? $province : 0;
            ...
            $sql = 'SELECT region_name FROM ' . $GLOBALS['ecs']->table('region')
                 . ' WHERE'//  region_id = ' . $country . ' OR
                 . ' region_id = ' . $province . ' OR region_id = ' . $city
                 . ' OR region_id = ' . $district . ' ORDER BY `parent_id`';
            ...
            foreach ($region as $name) {
                // 当前后两级地区名称相同时，不显示后一个地区
                if ($current_region != $name) {
                    $region_name_merge .= $name['region_name'];
                    $current_region = $name;
                }
            }
        }
    }
    return $region_name_merge;
}
?>
