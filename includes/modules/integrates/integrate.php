...
//        }
    ...
    function edit_user($cfg)
    {
        ...
        if (isset($cfg['nickname']) && $this->field_nickname != 'NULL')
        {
            $values[] = $this->field_nickname . "='" . $cfg['nickname'] . "'";
        }
        ...
        if ((isset($cfg['email_validated'])) && $this->field_email_validated != 'NULL')
        {
    ...
    function remove_user($id)
    {
        ...
        if ($this->need_sync || (isset($this->is_ecshop) && $this->is_ecshop))
        {
            ...
            if ($col)
            {
                ...
                $GLOBALS['db']->query($sql);               
                $sql = "DELETE FROM " . $GLOBALS['ecs']->table('users_third_info') . " WHERE " . db_create_in($col, 'user_id');//删除第三方用户登录资料
                $GLOBALS['db']->query($sql); 
            }
        }
