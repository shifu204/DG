...
function edit_profile($profile)
{
    ...
    if (isset($profile['email_validated']))
    {
        $cfg['email_validated'] = $profile['email_validated'];
    }
    ...
    /* 过滤非法的键值 */
    $other_key_array = array('msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone','mobile_validated','address_id');
...
function get_profile($user_id)
{
    ...
    $info  = array();
    $infos = array();
    $sql  = "SELECT u.*, ut.access_token, ut.headimgurl ".
    ...
    {
        $sql = "SELECT rank_id, rank_name, discount,rank_level FROM ".$GLOBALS['ecs']->table('user_rank') .
               " WHERE rank_id = '$infos[user_rank]'";
    }
    else
    {
        $sql = "SELECT rank_id, rank_name, discount, min_points,rank_level".
    ...
    if ($row = $GLOBALS['db']->getRow($sql))
    {
        ...
        $info['min_points']    = $row['min_points'];
    }
    else
    {
        $info['rank_name'] = $GLOBALS['_LANG']['undifine_rank'];
    }
    $info['rank_level'] = $row['rank_level'];
    $next_points = intval($infos['rank_points']);
    if($next_points == 0){
        $next_points = 1;
    }
    //获取下一等级的信息
    $sql = "SELECT rank_id, rank_name, discount, min_points,rank_level".
           " FROM ".$GLOBALS['ecs']->table('user_rank') .
           " WHERE min_points> " . $next_points . " ORDER BY min_points ASC";
    if($next_rank = $GLOBALS['db']->getRow($sql)){      
        $info['next_rank_level'] = $next_rank['rank_level'];
        $info['next_rank_name'] = $next_rank['rank_name'];
        $info['next_min_points'] = $next_rank['min_points'];
        $info['next_need_points'] = $next_rank['min_points'] - $infos['rank_points'];
        $info['next_discount'] = $next_rank['discount'] / 10;
    }
    ...
    $info['discount']    = $_SESSION['discount'] * 10;
    $info['user_name']   = $_SESSION['user_name'];
    $info['email']       = $_SESSION['email'];
    
    //格式化第三方登录的会员名
    if(!empty($infos['access_token'])){
    ...
    $info['reg_time']    = local_date('Y-m-d H:i:s',$infos['reg_time']);
    ...
    $info['last_login']  = $infos['last_login'];
    $info['headimgurl'] = $infos['headimgurl'];
    return $info;
}
...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    $sql = "SELECT order_id, order_sn, order_status, shipping_status, pay_status, add_time, invoice_no, " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        else if ($row['order_status'] == OS_SPLITED)
        {
            ...
            elseif ($row['shipping_status'] == SS_RECEIVED)
            {
                ...
                if (countNoShared($row['order_id']) > 0) {
                    @$row['handler'] .= '<br /><a href="user.php?act=shaidan_list&order_id=' . $row['order_id'] . '">'.$GLOBALS['_LANG']['share_order'] .'</a>';
                }
            }
            else
            {
        ...
        $row['invoice_no'] = $row['shipping_status'] == SS_SHIPPED_ING ? '' : $row['invoice_no'];
...
function countNoShared($order_id) {
    $sql = 'SELECT COUNT(og.is_share) FROM ' . $GLOBALS['ecs']->table('order_goods') . ' AS og'
         . ' LEFT JOIN ' . $GLOBALS['ecs']->table('order_info') . ' AS oi'
         . ' ON oi.order_id = og.order_id'
         . ' WHERE og.order_id = ' . $order_id . ' AND og.is_share = 0'
         . ' AND og.is_gift = 0 AND oi.shipping_time > ' . VALID_SHARE_ORDER_TIME;
    $count_no_shared = $GLOBALS['db']->getOne($sql);
    return $count_no_shared;
}
//获取用户总消费额
function get_user_total_used($user_id){
    //用户所有的订单
    $select = 'SUM(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee) AS order_total';
    $where = array('user_id'=>$user_id);
    $order_finished = get_order_finished_str();
    $total = $GLOBALS['cidb']->select($select)->where($where)->where($order_finished)->get('order_info')->row_array();
    return $total['order_total'];
}

function get_order_finished_str(){
    return "order_status >= ".OS_CONFIRMED." AND ( shipping_status = ".SS_SHIPPED ." OR shipping_status = ".SS_RECEIVED .") AND (pay_status = ".PS_PAYED ." OR pay_status = ".PS_PAYING.")";
}
//获取用户收藏商品
function get_user_collects($user_id){
    $select = "collect_goods.add_time, goods.goods_id, goods.goods_name, goods.is_promote, goods.goods_thumb";
    $where = array("collect_goods.user_id"=>$user_id);
    $collects = $GLOBALS['cidb']->select($select)->where($where)->join("goods","goods.goods_id = collect_goods.goods_id")->get("collect_goods")->result_array();
    return $collects;
}
?>
