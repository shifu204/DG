...
//    }
    ...
    else
    {
        ...
        //定义other合法的变量数组
        $other_key_array = array('msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone','nickname');
...
function edit_password($user_id, $old_password, $new_password='', $code ='')
{
    ...
    if($GLOBALS['user']->check_user($user_name, $old_password) > 0 ){
        $user_info = array();
        $user_info['username'] = $user_name;
        $user_info['password'] = $new_password;
        if ($GLOBALS['user']->edit_user($user_info))
        {
            $GLOBALS['user']->logout();
            return true;
        }
...
/**
 * 根据用户名获取用户相关信息（用于找回密码）
 */
function user_info_by_name($user_name){
    $sql = "SELECT user_id, email, mobile_phone FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_name = '$user_name'";
