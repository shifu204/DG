...
function get_collection_goods($user_id, $num = 10, $start = 0)
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, '.
                ...
                'g.promote_price, g.promote_start_date,g.promote_end_date, g.goods_thumb, c.rec_id, c.is_attention' .
...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT nickname, pay_points, user_money, credit_line, last_login, is_validated FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
    $info['username']  = stripslashes($_SESSION['user_name']);
    $info['nickname']  = $row['nickname'] ? $row['nickname'] : $info['username'];
 ...
 * 第三方登录帐户绑定ecshop帐户
 */
function third_bind_user($third_user = 0, $bind_user = 0){
    if(!empty($third_user) & !empty($bind_user)){
        //获取帐户信息    
        $sql = "SELECT u.*, ut.access_token, ut.headimgurl, ut.unionid FROM " . $GLOBALS['ecs']->table("users") . " AS u " .  
               "LEFT JOIN " . $GLOBALS['ecs']->table("users_third_info") . " AS ut ON ut.user_id = u.user_id WHERE u.user_id = ";
        $third_info = $GLOBALS['db']->getRow($sql . $third_user);
        $bind_info = $GLOBALS['db']->getRow($sql . $bind_user . " AND u.user_type = 0 ");
        if(!empty($third_info) && $bind_info){
            //计算帐户资金等
            $change_info = array();
            $change_info['user_money'] = floatval($third_info['user_money']) + floatval($bind_info['user_money']);
            $change_info['frozen_money'] = floatval($third_info['frozen_money']) + floatval($bind_info['frozen_money']);
            $change_info['pay_points'] = floatval($third_info['pay_points']) + floatval($bind_info['pay_points']);
            $change_info['rank_points'] = floatval($third_info['rank_points']) + floatval($bind_info['rank_points']);
            
            $change_info['email'] = empty($third_info['email'])? $bind_info['email'] : $third_info['email'];
            $change_info['user_type'] = $bind_info['user_type'];
            $change_info['user_name'] = $bind_info['user_name'];
            $change_info['nickname'] = empty($third_info['nickname'])? $bind_info['nickname'] : $third_info['nickname'];
            $change_info['visit_count'] = floatval($third_info['visit_count']) + floatval($bind_info['visit_count']);
            
            //帐户联系方式（已被绑定的帐户为准）
            $change_info['qq'] = empty($bind_info['qq'])? $third_info['qq']: $bind_info['qq'];
            $change_info['office_phone'] = empty($bind_info['office_phone'])? $third_info['office_phone']: $bind_info['office_phone'];
            $change_info['home_phone'] = empty($bind_info['home_phone'])? $third_info['home_phone']: $bind_info['home_phone'];
            $change_info['mobile_phone'] = empty($bind_info['mobile_phone'])? $third_info['mobile_phone']: $bind_info['mobile_phone'];
            
            // 更新绑定帐户信息
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('users'), $change_info, 'UPDATE', 'user_id = ' . $bind_user);
            //从用户表中删除第三方用户帐户，备份到用户帐户备份表
            unset($third_info['access_token']);
            unset($third_info['headimgurl']);
            unset($third_info['unionid']);
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('users_bak'), $third_info, 'INSERT');
            $sql = "DELETE FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id = " . $third_user . ' LIMIT 1';
            $GLOBALS['db']->query($sql);
            
            //更新第三方帐户登录信息表
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('users_third_info'), array('user_id'=>$bind_user, 'ori_id'=>$third_user), 'UPDATE', 'user_id = ' . $third_user);
            //更新和用户相关的其它数据库表
            update_user_relate_tables($third_user, $bind_user);
        }
    }
}

/*
 * 第三方用户绑定帐户时，更新和用户有关的数据库表
 */
function update_user_relate_tables($third_user, $bind_user){
    //相关数据表名数组
    $tables = array('account_log', 'back_order', 'booking_goods', 'comment', 'collect_goods', 'delivery_order', 'feedback', 'order_info', 'reg_extend_info', 'user_account', 'user_address', 'user_bonus', 'user_feed');
    if($third_user && $bind_user){
        foreach($tables as $table){
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table($table), array('user_id'=>$bind_user), 'UPDATE', 'user_id = ' . $third_user);
        }
    }
}
...
function get_user_full($user_id){
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table("users") . " AS u " .  
           "LEFT JOIN " . $GLOBALS['ecs']->table("users_third_info") . " AS ut ON ut.user_id = u.user_id WHERE u.user_id = " .$user_id;
    return $GLOBALS['db']->getRow($sql);
}
?>
