...
class cls_template
{
    ...
    {
		// @hero:判断是否手机版，如果是的话就加载手机模板
		global $mobile_templete;
		if (!empty($mobile_templete)) {
			$filename_m = str_replace('.dwt', $mobile_templete . '.dwt', $filename);
			if (file_exists($this->template_dir . '/' . $filename_m)) {
				$filename = $filename_m;
			}
		}
