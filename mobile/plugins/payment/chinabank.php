...
class chinabank
{
    ...
    function get_code($order, $payment)
    {
        ...
        $def_url  = '<br /><form style="text-align:center;" method=post action="https://pay3.chinabank.com.cn/PayGate" target="_blank" id="pay_form">';
        ...
        $def_url .= '<input type=image title="' .$GLOBALS['_LANG']['pay_button']. '" src="http://www.deebei.net/js/wangyin.jpg" style="cursor:pointer;border:0px;width:220px;height:42px;">';
        $def_url .= "</form>";
        $this->pay_href = $def_url;
        return $def_url;
    }
    
    function get_pay_href($order, $payment){
        $return = array();
        $data_vid           = trim($payment['chinabank_account']);
        $data_orderid       = $order['order_sn'];
        $data_vamount       = $order['order_amount'];
        ...
        $MD5KEY =$data_vamount.$data_vmoneytype.$data_orderid.$data_vid.$data_vreturnurl.$data_vpaykey;
        $MD5KEY = strtoupper(md5($MD5KEY));
        $form_id = "pay_form_".$order['order_sn'];
        $def_url  = '<form style="text-align:center;" method=post action="https://pay3.chinabank.com.cn/PayGate" target="_blank" id="'.$form_id.'">';
        $def_url .= "<input type=HIDDEN name='v_mid' value='".$data_vid."'>";
        $def_url .= "<input type=HIDDEN name='v_oid' value='".$data_orderid."'>";
        $def_url .= "<input type=HIDDEN name='v_amount' value='".$data_vamount."'>";
        $def_url .= "<input type=HIDDEN name='v_moneytype'  value='".$data_vmoneytype."'>";
        ...
        $def_url .= "<input type=HIDDEN name='remark1' value='".$remark1."'>";
        //$def_url .= "<input type=submit value='" .$GLOBALS['_LANG']['pay_button']. "'>";
        $def_url .= '<a id="pay_order_button"  title="' .$GLOBALS['_LANG']['pay_button']. '"  class="pay_order_btn_m" href="javascript:document.getElementById(\''.$form_id.'\').submit();"></a>';
        $def_url .= "</form>";
        $this->pay_href = $def_url;
        return $def_url;
    }
