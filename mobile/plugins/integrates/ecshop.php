...
class ecshop extends integrate
{
     ...
     */
    public function check_user($username, $password = null, $from = null)
    {
    ...
    function check_third_user($post_username,$password, $from){
        ...
        $sql = "SELECT user_id, password, salt,ec_salt " .
               " FROM " . model('Base')->model->pre . $this->user_table .
               " WHERE access_token = '$access_token'";
        $row = model('Base')->model->getRow($sql);
        $ec_salt=$row['ec_salt'];
        if (empty($row['salt']))
        {
            ...
            else
            {
                if(empty($ec_salt))
                {
                    ...
                    $sql = "UPDATE " . model('Base')->model->pre . $this->user_table . " SET password= '" .$new_password."',ec_salt='".$ec_salt."'".
                           " WHERE access_token = '$access_token'";
                    model('Base')->model->query($sql);
                }
                return $row['user_id'];
        ...
        else
        {
            ...
            $sql = "UPDATE " . model('Base')->model->pre . $this->user_table .
    ...
    function set_session ($username='', $from = '')
    {
        ...
        else
        {
            ...
            $sql = "SELECT u.user_id, u.password, u.email, u.nickname FROM " . model('Base')->model->pre . "users AS u ".
                   " LEFT JOIN " . model('Base')->model->pre . "users_third_info AS ut ON ut.user_id = u.user_id".
    ...
    function set_cookie($username='', $remember= null, $from = '')
    {
        ...
        elseif ($remember)
        {
            ...
            $sql = "SELECT user_id, password FROM " . model('Base')->model->pre . "users WHERE $field = '$username' LIMIT 1";
            $row = model('Base')->model->getRow($sql);
            if ($row)
            {
