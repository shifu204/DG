...
class GoodsModel extends BaseModel {
    ...
    function get_goods_info($goods_id) {
        ...
        if ($row !== false) {
            ...
            if(!empty($row['goods_same_id'])){
                $sql = "SELECT goods_thumb, goods_img, original_img, goods_index_img, goods_desc FROM ".$this->pre."goods WHERE goods_id = {$row['goods_same_id']}";
                $same = $this->row($sql);
                $row['goods_img'] = $same['goods_img'];
                $row['goods_thumb'] = $same['goods_thumb'];
                $row['original_img'] = $same['original_img'];
                $row['goods_index_img'] = $same['goods_index_img'];
                //商品描述
                $row['goods_desc'] = $same['goods_desc'];
            }
            return $row;
        } else {
            return false;
    ...
    /*
     * 获取商品的same_goods(商品套装)
     */
    function get_same_goods($goods_id, $goods_info = array()){
        if(empty($goods_info)){
            $goods_info = $this->get_goods_info($goods_id);
        }
        $sql = "SELECT goods_id, cat_id, goods_name, brand_id, shop_price, promote_price, promote_start_date, promote_end_date, goods_desc, goods_thumb, goods_img, "
                . "original_img, is_buy_six, is_buy_nine, is_total_four " . 
                " FROM " . $this->pre . "goods" . 
                " WHERE goods_same_id = $goods_id AND goods_id != $goods_id " . 
                " ORDER BY sort_order";
        $res = $this->query($sql);
        $arr = array();
        foreach($res as $goods){
            ...
            $goods['promote_price_formated'] = price_format($promote_price);
            $goods['shop_price_formated'] = price_format($goods['shop_price']);
            if(empty($goods['goods_thumb'])){
                $goods['goods_thumb'] = $goods_info['goods_thumb'];
            }
            if(empty($goods['goods_img'])){
                $goods['goods_img'] = $goods_info['goods_img'];
            }
            if(empty($goods['original_img'])){
                $goods['original_img'] = $goods_info['original_img'];
            }
