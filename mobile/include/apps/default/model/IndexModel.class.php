...
class IndexModel extends CommonModel {
    ...
    public function goods_list($type = 'best', $limit = 10, $start = 0) {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, g.is_buy_six, g.is_buy_nine, g.is_total_four, ' . "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, " . "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, RAND() AS rnd " . 'FROM ' . $this->pre . 'goods AS g ' . "LEFT JOIN " . $this->pre . "member_price AS mp " . "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ";
        ...
        foreach ($result as $key => $vo) {
            ...
            $goods[$key]['mysc'] = 0;
            $goods[$key]['is_buy_nine'] = $vo['is_buy_nine'];
            $goods[$key]['is_buy_six'] = $vo['is_buy_six'];
            $goods[$key]['is_total_four'] = $vo['is_total_four'];
            // 检查是否已经存在于用户的收藏夹
            if ($_SESSION ['user_id']) {
                // 用户自己有没有收藏过
