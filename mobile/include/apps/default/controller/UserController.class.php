...
class UserController extends CommonController {
    ...
    public function __construct() {
        ...
        $this->check_login();
        // 用户信息
        $info = model('Clips')->get_user_default($this->user_id);
        // 如果是显示页面，对页面进行相应赋值
        assign_template();
    ...
    public function register() {
        ...
        if (IS_POST) {
            ...
            //检测手机是否唯一
            if(!model('DebeiUsers')->check_mobile($mobile)){
                show_message(L('mobile_error'), L('register_back'), url('register'), 'error');
            }
            $other['mobile_phone'] = $mobile;
            if (model('DebeiUsers')->register($username, $password, $email, $other) !== false) {
                // 判断是否需要自动发送注册邮件
    ...
    public function third_login() {
        ...
        if($type == 'qq'){
            $url = __URL__ . '/index.php/default/user/third_login/type/qq.html';
        } else {
        ...
        if ($_GET['code'] && $_GET['code'] != '') {
            ...
            if ($obj->call_back($info, $url, $_GET['code'])) {
                if ($_SESSION['access_token']) {
                    ...
                    if($type == 'qq'){
                        $info['ori_user_name'] = $info['user_name'];
                        //$info['user_name'] = $openid.'@tencent';
                    }
                    $info['openid'] = $openid;
                    $info['type'] = $type;
                    if (model('DebeiUsers')->get_one_user($info)) {
                        // 已有记录
                        self::$user->set_session($openid, $type);
                        self::$user->set_cookie($openid, null, $type);
                        model('Users')->update_user_info();
                        model('Users')->recalculate_price();
                        $jump_url = empty($this->back_act) ? url('index') : $this->back_act;
                        $this->redirect($jump_url);
...
//                    }
                    $info['email'] = empty($user['email']) ? '': $user['email'];
                    // 插入数据库
                    model('DebeiUsers')->third_reg($info);
                    self::$user->set_session($openid, $type);
                    self::$user->set_cookie($openid, null, $type);
                    model('Users')->update_user_info();
                    model('Users')->recalculate_price();
