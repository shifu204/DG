...
class CategoryController extends CommonController {
    ...
    public function top_all() {
        ...
        if (!ECTouch::view()->is_cached('category_all.dwt', $cache_id)) {
            $category = model('DebeiCategoryBase')->get_filter_category();
    ...
    private function category_get_goods() {
        ...
        /* 获得商品列表 */
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, g.is_buy_six, g.is_buy_nine, g.is_total_four, g.goods_same_id, ' . "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, " . 'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img, xl.sales_volume ' . 'FROM ' . $this->model->pre . 'goods AS g ' . ' LEFT JOIN ' . $this->model->pre . 'touch_goods AS xl ' . ' ON g.goods_id=xl.goods_id ' . ' LEFT JOIN ' . $this->model->pre . 'member_price AS mp ' . "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " . "WHERE $where $this->ext ORDER BY $sort $this->order, sort_order ASC LIMIT $start , $this->size";
        $res = $this->model->query($sql);
        $arr = array();
        foreach ($res as $row) {
            // 销量统计
            ...
            if(!empty($row['goods_same_id'])){
                $sql = "SELECT goods_thumb, goods_img, original_img, goods_index_img FROM ".$this->model->pre."goods WHERE goods_id = {$row['goods_same_id']}";
                $same = $this->model->getRow($sql);
                $arr[$row['goods_id']]['goods_img'] = $same['goods_img'];
                $arr[$row['goods_id']]['goods_thumb'] = $same['goods_thumb'];
                $arr[$row['goods_id']]['original_img'] = $same['original_img'];
                $arr[$row['goods_id']]['goods_index_img'] = $same['goods_index_img'];
            }
        }
        return $arr;
    }
