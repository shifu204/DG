...
if (empty($_REQUEST['act']))
{
...
else if($_REQUEST['act'] == "shaidan_zan"){
    $result = array();
    $result['error'] = '';
    ...
    if($_SESSION['user_id'] > 0 ){
        if($comment_id > 0){
            //获取该用户是否已经点了赞
            $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('comment_data') . " WHERE comment_id = $comment_id AND re_user_id = {$_SESSION['user_id']} ";
            $comment_info = $GLOBALS['db']->getRow($sql);
            if(empty($comment_info) || $comment_info['love_click'] == 0){                          
                $sql = "UPDATE ".$GLOBALS['ecs']->table('comment'). " SET lovenum = lovenum + 1 WHERE comment_id = ".$comment_id ." LIMIT 1";
                $res = $GLOBALS['db']->query($sql);
                if(!$res){          
                    $result['error'] = 1;
                } else {
                    //获取赞的数量
                    $msg = $GLOBALS['db']->getOne("SELECT lovenum FROM ".$GLOBALS['ecs']->table('comment')." WHERE comment_id = ".$comment_id);
                    $result['lovenum'] = $msg;
                    //更新用户评论信息表
                    $now = gmtime();
                    if(empty($comment_info)){
                        ...
                        $insert['re_user_id'] = $_SESSION['user_id'];
                        $insert['love_click'] = 1;
                        $insert['add_time'] = $now;
                        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('comment_data'), $insert);
                    } else {
                        $update = array();
                        $update['love_click'] = $comment_info['love_click'] + 1;
                        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('comment_data'), $update, 'UPDATE', "comment_id = $comment_id AND re_user_id = {$_SESSION['user_id']} ");
                    }
                }
            } else {
                $result['error'] = 3;
...
else
{
    ...
    $cmt->type = !empty($_GET['type']) ? trim($_GET['type']) : 0;
