<?php
			...
			$sql="SELECT user_name,password FROM ". $ecs->table('admin_user') ."WHERE user_name = '" .$admin_user. "'";
		...
		elseif ($action=='d')
		    ...
		    $now_time = local_date('Y-m-d H:i:s', time());
				...
				foreach ($order_ids as $val) {
				    ...
				    $goods_list = array();
				    $goods_attr = array();
    	   $sql = "SELECT o.*, IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, g.suppliers_id, IFNULL(b.brand_name, '') AS brand_name,s.suppliers_name,g.goods_thumb,g.goods_img
		...
		elseif ($action=='f')
	  	{
			  ...
			  /*开始*/
			   ...
			   if (!empty($invoice_no))
                ...
                $action_note = ' xunjie Send !'.$_POST['V'];			 
