...
elseif (isset($_POST['Submit']))
{
    ...
    /*pay_logo上传图片*/
    if(!empty($_FILES['pay_logo'])){
        include_once(ROOT_PATH . '/includes/cls_image.php');
        $image = new cls_image();
        if($image->check_img_type($_FILES['pay_logo']['type'])){
            $pay_logo = $image->upload_image($_FILES['pay_logo'], "pay_logo",$_POST['pay_code'].'.jpg');
        }
    }
    if(!$pay_logo) {
        $pay_logo = 'no_picture.gif';
    }
    /* 检查是编辑还是安装 */
    ...
    else
    {
        ...
        else
        {
            ...
            $sql = "INSERT INTO " . $ecs->table('payment') . " (pay_code, pay_name, pay_desc, pay_config, is_cod, pay_fee, enabled, is_online,pay_logo)" .
                   "VALUES ('$_POST[pay_code]', '$_POST[pay_name]', '$_POST[pay_desc]', '$pay_config', '$_POST[is_cod]', '$pay_fee', 1, '$_POST[is_online]','$pay_logo')";
            $db->query($sql);
        }
