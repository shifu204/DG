...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        if (isset($_REQUEST['is_ajax']) && $_REQUEST['is_ajax'] == 1)
        {
            ...
            $filter['nickname'] = json_str_iconv($filter['nickname']);
        }
        ...
        $filter['mobile_phone'] = empty($_REQUEST['mobile_phone']) ? 0 : intval($_REQUEST['mobile_phone']);
        ...
        if($filter['nickname']){
            $ex_where .= " AND nickname LIKE '%" . mysql_like_quote($filter['nickname']) ."%'";
        }
        ...
        $sql = "SELECT user_id, user_name, nickname, email, user_type, is_validated, user_money, frozen_money, rank_points, pay_points, ec_salt, reg_time, mobile_phone,last_login ".
    ...
    foreach ($user_list as $key => $value) {
        // 处理第三方登录的会员名字
        //$user_list[$key]['user_name'] = $value['user_type'] ? $value['nickname'] : $value['user_name'];
        $user_list[$key]['keyword'] = trace::get_user_keyword($value['user_id']);
        $user_list[$key]['formated_last_login'] = local_date("Y-m-d H:i:s", $value['last_login']);
    }
