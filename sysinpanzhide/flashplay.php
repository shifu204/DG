...
//        }
    ...
    $ad_status_COUNT = $GLOBALS['db']->getOne($sql);
	$smarty->assign('ad_status_COUNT', $ad_status_COUNT);
...
elseif ($_REQUEST['act'] == 'custom_insert')
{
	...
	//接收三图文件
	$filter['ad']['ad_minimg'] = '';
    if ($ad_img['ad_img_url']['name'] && $ad_img['ad_img_url']['size'] > 0)
    {
        /* 检查文件合法性 */
        if(!get_file_suffix($ad_img['ad_img_url']['name'], $allow_suffix))
        {
            sys_msg($_LANG['invalid_type']);
        }
        ...
        $name .= '.' . end(explode('.', $ad_img['ad_img_url']['name']));
        $target = ROOT_PATH . DATA_DIR . '/afficheimg/' . $name;

        if (move_upload_file($ad_img['ad_img_url']['tmp_name'], $target))
        {
            $filter['ad']['ad_minimg'] = DATA_DIR . '/afficheimg/' . $name;
        }
    }
    ...
    $filter['ad']['ad_color'] = trim($_REQUEST['ad']['ad_color']);
    $ad = array('ad_type' => $filter['ad']['ad_type'],
                ...
                'ad_color'=>$filter['ad']['ad_color']
               );
	
	if($filter['ad']['ad_minimg']){
		$ad['ad_minimg'] = $filter['ad']['ad_minimg'];
		$ad['ad_url1'] = $filter['ad']['ad_url1'];
		$ad['ad_url2'] = $filter['ad']['ad_url2'];
		$ad['ad_url3'] = $filter['ad']['ad_url3'];
		$ad['ad_sort'] = intval($filter['ad']['ad_sort']);
	}
...
elseif ($_REQUEST['act'] == 'custom_edit')
{
    ...
    /* 查询自定义广告信息 */
    $sql = "SELECT ad_id, ad_type, content, url, ad_status, ad_name,ad_minimg,ad_url1,ad_url2,ad_url3,ad_sort, ad_color FROM " . $GLOBALS['ecs']->table("ad_custom") . " WHERE ad_id = $id LIMIT 0, 1";
...
elseif ($_REQUEST['act'] == 'custom_colse'){
	clear_all_files();
	
	if(intval($_GET['iscolse'])){
		/* 关闭 所有自定义广告 */
		$GLOBALS['db']->query("UPDATE " . $GLOBALS['ecs']->table("ad_custom") . " SET ad_status = 0 WHERE ad_status=1");
		
		$sql = "UPDATE " . $GLOBALS['ecs']->table("shop_config") . " SET value = 'sys' WHERE id =337";
		$GLOBALS['db']->query($sql);
	}else{
		/* 开启 所有自定义广告 */
		$GLOBALS['db']->query("UPDATE " . $GLOBALS['ecs']->table("ad_custom") . " SET ad_status = 1 WHERE ad_status=0");
...
function ad_list()
{
    ...
    if ($result === false)
    {
		...
		$tmp = $GLOBALS['db']->getRow('Describe ' . $GLOBALS['ecs']->table('ad_custom') . ' ad_minimg');
		if(!$tmp){
			$GLOBALS['db']->query('ALTER TABLE ' . $GLOBALS['ecs']->table('ad_custom') . ' '.
			'ADD COLUMN `ad_minimg` varchar(255) NULL AFTER `ad_status`,'.
			'ADD COLUMN `ad_url1` varchar(255) NULL AFTER `ad_minimg`,'.
			'ADD COLUMN `ad_url2` varchar(255) NULL AFTER `ad_url1`,'.
			'ADD COLUMN `ad_url3` varchar(255) NULL AFTER `ad_url2`'.
			'ADD COLUMN `ad_sort` int(10) NULL DEFAULT 0 AFTER `ad_url3`'
			);
		}
		
		$aiax = isset($_GET['is_ajax']) ? $_GET['is_ajax'] : 0;
        $filter = array(); 
        $filter['sort_by'] = 'ad_sort,add_time';
        ...
        $sql = "SELECT ad_id, CASE WHEN ad_type = 0 THEN '图片'
                                   ...
                                   ELSE '' END AS type_name, ad_name, add_time, CASE WHEN ad_status = 1 THEN '启用' ELSE '关闭' END AS status_name, ad_type, ad_status, ad_sort 
