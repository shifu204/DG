...
//-- 编辑红包领取数量
/*------------------------------------------------------ */
if($_REQUEST['act'] == 'edit_get_num'){
    $id = intval($_POST['id']);
    $val = floatval($_POST['val']);
    if ($val < 0)
    {
        make_json_error($_LANG['get_num_empty']);
    }
    else {
        $exc->edit("get_num='$val'", $id);
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $get_num     = !empty($_POST['get_num']) ? intval($_POST['get_num']) : 0;
    ...
    /* 插入数据库。 */
    $sql = "INSERT INTO ".$ecs->table('bonus_type')." (type_name, type_money,send_start_date,send_end_date,use_start_date,use_end_date,send_type,min_amount,min_goods_amount,get_num)
    VALUES ('$type_name',
            '$_POST[type_money]',
            '$send_startdate',
            '$send_enddate',
            ...
            '$use_enddate',
            '$_POST[send_type]',
            '$min_amount','" . floatval($_POST['min_goods_amount']) . "','$get_num')";
...
if ($_REQUEST['act'] == 'send')
{
    ...
    elseif ($_REQUEST['send_by'] == SEND_BY_REGISTER){
        $smarty->assign('id',           $id);
        $smarty->assign('ranklist',     get_rank_list());

        $smarty->display('bonus_by_register.htm');
    }
...
if($_REQUEST['act'] == 'send_by_register'){
    $user_array = (is_array($_REQUEST['user'])) ? $_REQUEST['user'] : explode(',', $_REQUEST['user']);
    ...
    foreach($user_array AS $uid){
        ...
        $sql = "INSERT INTO " . $ecs->table('user_bonus') .
                "(bonus_type_id, bonus_sn, user_id, used_time, order_id, emailed) " .
                "VALUES ('$_REQUEST[id]', 0, '$uid', 0, 0, 0)";
        $db->query($sql);
    }
    $link[] = array('text' => $_LANG['back_list'], 'href' => 'bonus.php?act=list');
