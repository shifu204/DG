...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    if ($order['user_id'] > 0)
    {
        ...
        if (!empty($user))
        {
            $order['user_name'] = $user['user_type'] ? $user['ec_salt'] : $user['user_name'];
        }
    }
    ...
    $order['invoice_no']    = $order['shipping_status'] == SS_UNSHIPPED ? $_LANG['ss'][SS_UNSHIPPED] : $order['invoice_no'];// || $order['shipping_status'] == SS_PREPARING
    $order['invoice_no_str'] = $order['invoice_no'];
    $order['invoice_no']    = explode('<br>', $order['invoice_no']);
    $order['un_invoice_no'] = getUnInvoiceNo($order['order_id']);
    ...
    elseif (isset($_GET['shipping_print']))
    {
        ...
        $smarty->assign('shop_address', $_CFG['shop_address']);
        $smarty->assign('service_phone',$_CFG['service_phone']);
        if ($order['shipping_id'] == 17 || $order['shipping_name'] == '普通快递') {
            $print_where = 'shipping_name = \'' . $order['shipping_note'] . '\'';
        } else {
            $print_where = 'shipping_id = ' . $order['shipping_id'];
        }
        $shipping = $db->getRow("SELECT * FROM " . $ecs->table("shipping") . " WHERE " . $print_where);
        FB::info($shipping);
        ...
        if ($shipping['print_model'] == 2)
        {
            ...
            $lable_box['t_customer_address'] = $region_array[$order['province']]
                . $region_array[$order['city']] . $region_array[$order['district']]
                . $order['address']; //收件人-详细地址
            ...
            $lable_box['t_to_buy'] = $order['to_buyer'];
            $lable_box['t_month_code'] = '7574349678';

            $order['is_cod'] = ($order['pay_id'] == 3) ? 1 : 0;
            $lable_box['t_is_cod'] = $order['is_cod'] ? '√' : '';
            $lable_box['t_goods_amount'] = $_GET['goods_amount'] ? $_GET['goods_amount'] : $order['goods_amount'];
            $lable_box['t_goods_amount'] = $order['is_cod'] ? $lable_box['t_goods_amount'] : '';
            $lable_box['t_cod_code']       = $order['is_cod'] ? '7574425030' : '';

            $lable_box['t_pigeon'] = '√'; //√-对号
            $lable_box['t_pigeon2'] = '√'; //√-对号
            $lable_box['t_pigeon3'] = '√'; //√-对号
            $lable_box['t_pigeon4'] = '√'; //√-对号
            $lable_box['t_pigeon5'] = '√'; //√-对号
            ...
            $shipping['config_lable'] = str_replace("\n",'<br>',str_replace("\r",'<br>',str_replace("\r\n",'<br>',$shipping['config_lable'])));
            FB::info($shipping);

            $smarty->assign('shipping', $shipping);
            $is_preview = $_GET['is_preview'] ? $_GET['is_preview'] : 0;
...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    if (($require_note && $action_note == '') || isset($show_invoice_no) || isset($show_refund))
    {
        ...
        $smarty->assign('mobile', $mobile); // 手机号码
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('split' == $operation)
    {
        ...
        // @hero: 将订单号提前写到发货单中
        $delivery['invoice_no'] = $_REQUEST['invoice_no'];

        // 同时把「发货单号」更新到订单中（现在不用了）
        //$insert_arr['invoice_no'] = $_REQUEST['invoice_no'];
        //update_order($order_id, $insert_arr);
        ...
        $filter_fileds = array(
                               ...
                               'suppliers_id', 'status', 'order_id', 'shipping_name', 'invoice_no'
                               );
        $_delivery = array();
...
//-- 编辑普通快递备注
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_shipping_note')
{
    /* 检查权限 */
    check_authz_json('order_edit');

    $no = empty($_REQUEST['val']) ? 'N/A' : json_str_iconv(trim($_REQUEST['val']));
    $no = $no=='N/A' ? '' : $no;
    $order_id = empty($_REQUEST['id']) ? 0 : intval($_REQUEST['id']);
    ...
    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('order_info') . " SET shipping_note='$no' WHERE order_id = '$order_id'";
    if ($GLOBALS['db']->query($sql))
    {
        if (empty($no))
        {
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 查询 */
        $sql = "SELECT o.user_id, o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.shipping_name, o.shipping_note, o.pay_id, o.is_remind_pay, o.order_amount, o.money_paid, o.invoice_no, o.is_remind_express, o.pay_note, " .
                    "o.pay_status, o.consignee, o.address, o.email, o.tel, o.mobile, o.extension_code, o.extension_id, o.is_mobile, " .
                    "(" . order_amount_field('o.') . ") AS total_fee, " .
                    "IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer ".
                    ",u.user_type, u.ec_salt ".
                " FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
                " LEFT JOIN " .$GLOBALS['ecs']->table('users'). " AS u ON u.user_id=o.user_id ". $where .
                " ORDER BY $filter[sort_by] $filter[sort_order] ".
                " LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";

        foreach (array('order_sn', 'consignee', 'email', 'address', 'zipcode', 'tel', 'mobile', 'user_name') AS $val)
        {
    ...
    $count_history_order = getCountHistoryOrder();
    ...
    foreach ($row AS $key => $value)
    {
        ...
        if ($value['shipping_name'] == '普通快递') {
            $row[$key]['shipping_name'] = $value['shipping_note'];
        }
        //@hero
        $row[$key]['tel'] = !empty($value['mobile']) ? $value['mobile'] : $value['tel'];
        $row[$key]['buyer'] = $value['user_type'] ? $value['ec_salt'] : $value['buyer'];
        $row[$key]['count_history_order'] = $count_history_order[$value['user_id']];
        $row[$key]['invoice_no'] = str_replace('<br>', ', ', $value['invoice_no']);
        $row[$key]['un_invoice_no'] = getUnInvoiceNoList($value['order_id']);
        
        //查看是否从搜索引擎进来的订单
        $row[$key]['search_keyword'] = trace::get_order_keyword($value['order_sn']);
        FB::Error($row[$key]['un_invoice_no']);

        /* 取得区域名 */
        $row[$key]['region'] = getRegionName($value['order_id']);
    }
...
function delivery_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT delivery_id, delivery_sn, order_sn, order_id, add_time, action_user, consignee, country,
                       province, city, district, tel, mobile, status, update_time, email, suppliers_id
                FROM " . $GLOBALS['ecs']->table("delivery_order") . "
                $where
                ORDER BY " . $filter['sort_by'] . " " . $filter['sort_order']. "
                LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ", " . $filter['page_size'] . " ";
...
function back_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT back_id, delivery_sn, order_sn, order_id, add_time, action_user, consignee, country,
                       province, city, district, tel, mobile, status, update_time, email, return_time
...
function getUnInvoiceNo($order_id) {
    $sql = 'SELECT invoice_no FROM ' . $GLOBALS['ecs']->table('delivery_order')
         . ' WHERE order_id = ' . $order_id . ' AND status = ' . DS_NORMAL;
    $result = $GLOBALS['db']->getAll($sql, 1);
    FB::info($result);
    $un_invoice_no_arr = array();
    foreach ($result as $key => $value) {
        $un_invoice_no_arr = array_merge($un_invoice_no_arr, explode('<br>', $value));
    }
    return $un_invoice_no_arr;
}

function getUnInvoiceNoList($order_id) {
    $sql = 'SELECT delivery_id, invoice_no FROM ' . $GLOBALS['ecs']->table('delivery_order')
         . ' WHERE order_id = ' . $order_id . ' AND status = ' . DS_NORMAL;
    $un_invoice_no = $GLOBALS['db']->getAll($sql);
    foreach ($un_invoice_no as $key => $value) {
        $un_invoice_no[$key]['invoice_no'] = str_replace('<br>', ', ', $value['invoice_no']);
    }
    return $un_invoice_no;
}
