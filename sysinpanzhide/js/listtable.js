...
listTable.edit = function(obj, act, id)
{
  ...
  var txtWidth = obj.offsetWidth;
  if(txtWidth < 24){
      txtWidth = 24;
  }
  txt.style.width = (txtWidth + 12) + "px" ;
  ...
  txt.onblur = function(e)
  {
    ...
    {     
      if(typeof(id) == 'object'){
          id.val = encodeURIComponent(Utils.trim(txt.value));
          res = Ajax.call(listTable.url+'&act='+act, id , null, "POST", "JSON", false);
      }else{
...
/**
 * jquery的listTable.edit
 */
listTable.editJquery = function(obj, act, id,need_data)
{
  var parent = $(obj).parent();
  var org_obj = obj;
  var input_num = $(obj).find("input").length;
  var select_num = $(obj).find("select").length;
  if(input_num || select_num){
      return;
  }
  /* 保存原始的内容 */
  var org = $(obj).html();
 ...
 if($(obj).children().first().length != 0){
  ...
  var inputWidth = obj.offsetWidth;
  if(inputWidth < 80){
      inputWidth = 80;
  }
  ...
  if(need_data){
      ...
      var res = Ajax.call(listTable.url+'&act=edit_jquery_data', id , null, "POST", "JSON", false);
      if(res.is_select){
          input = $("<select></select>");
          $.each(res.content,function(i,j){
              var option = "<option>"+j+"</option>";
              $(input).append(option);
          });
      }
  }
  ...
  $(input).blur(function(){
    ...
    {     
      if(typeof(id) == 'object'){
        ...
        res = Ajax.call(listTable.url, "act="+act+"&val=" + $(input).val() + "&id=" +id, null, "POST", "JSON", false);
      }
