...
$exc = new exchange($ecs->table("sms_templates"), $db, 'template_id', 'template_subject');
...
if($_REQUEST['act'] == 'sms_template'){
    ...
    $smarty->display('sms/all_sendsms.htm');
}
else if($_REQUEST['act'] == 'load_template'){
    /* 获取模板短信信息 */
    $dddd=$_REQUEST['tpl'];
    $sql = "select * from " .$GLOBALS['ecs']->table('sms_templates') . " where template_id = ".$_REQUEST['tpl'];
...
//    $sql = "select template_id, template_subject, template_content from ".$ecs->table('sms_templates'). " where template_id = ".$_REQUEST['tpl'];
//    $sms = $db->GetRow($sql);
    ...
    $is_only = $exc->is_only('template_subject', $_POST['template_name']);
    if(!$is_only){
        sys_msg(sprintf($_LANG['template_subject_exist'], stripslashes($_POST['template_name'])), 1);
    }
    
    $sql = "insert into ".$ecs->table('sms_templates')."(template_user,template_subject,template_content) values ('$_POST[template_user]','$_POST[template_name]','$_POST[template_desc]')";
    $db->query($sql);
    
    $link[0]['text'] = $_LANG['templates_add'];
    $link[0]['href'] = 'sms.php?act=add';
    
    $link[1]['text'] = $_LANG['back_list'];
    $link[1]['href'] = 'sms.php?act=sms_template';
    
    clear_cache_files(); //清除缓存
    
    sys_msg($_POST['template_name'].$_LANG['template_succed'],0,$link);
    ...
    $smarty->assign('ur_here',     $_LANG['sms_add']);
    $smarty->assign('action_link', array('text' => $_LANG['07_sms_template'], 'href' => 'sms.php?act=sms_template'));
    ...
    $sql = "select template_id, template_user , template_subject, template_content from ".$ecs->table('sms_templates'). " where template_id='$_REQUEST[id]'";
    $sms = $db->GetRow($sql);
    
    $smarty->assign('sms',         $sms);
    
    $smarty->assign('ur_here',     $_LANG['sms_edit']);
    $smarty->assign('action_link', array('text' => $_LANG['07_sms_template'], 'href' => 'sms.php?act=sms_template'));
...
//    检查重名
    if($_POST['template_name'] != $_POST['old_template_name']){
        $is_only = $exc->is_only('template_subject', $_POST['template_name']);
        if(!$is_only){
            sys_msg(sprintf($_LANG['template_subject_exist'], stripslashes($_POST['template_name'])), 1);
        }
    ...
    if($exc->edit("template_user ='$_POST[template_user]', template_subject ='$_POST[template_name]', template_content ='$_POST[template_desc]'", $_POST['id'])){
        $link[0]['text'] = $_LANG['back_list'];
        $link[0]['href'] = 'sms.php?act=sms_template';
        $note = sprintf($_LANG['template_edit_succed'], $_POST['template_name']);
        clear_cache_files();
        sys_msg($note, 0, $link);   
    }
    ...
    $link[0]['text'] = $_LANG['back_list'];
    $link[0]['href'] = 'sms.php?act=sms_template';
    sys_msg($_LANG['template_remove'],0,$link);
...
function get_all_template(){
    /* 获得所有短信模板 */
    $sql = "SELECT template_id, template_subject FROM " .$GLOBALS['ecs']->table('sms_templates') . "";
    $res = $GLOBALS['db']->query($sql);
    while ($row = $GLOBALS['db']->FetchRow($res))
    {
        ...
        $templates[$row['template_id']] = $row[template_subject];
    }
    return $templates;
}
