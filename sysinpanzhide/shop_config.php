...
if ($_REQUEST['act'] == 'list_edit')
{
	...
	$_LANG['cfg_name']['logintojf'] = '送登录积分值';
	if(!$GLOBALS['db']->getOne("SELECT id FROM " .$GLOBALS['ecs']->table('shop_config'). " WHERE code='logintojf'")){
		$GLOBALS['db']->query("INSERT INTO ".$GLOBALS['ecs']->table('shop_config')."(parent_id, code, type, value, sort_order)VALUES('2','logintojf','text','30','1')");
	}

	$_LANG['cfg_name']['shaidanjf'] = '送晒单积分值';
	if(!$GLOBALS['db']->getOne("SELECT id FROM " .$GLOBALS['ecs']->table('shop_config'). " WHERE code='shaidanjf'")){
		$GLOBALS['db']->query("INSERT INTO ".$GLOBALS['ecs']->table('shop_config')."(parent_id, code, type, value, sort_order)VALUES('2','shaidanjf','text','30','1')");
	}
