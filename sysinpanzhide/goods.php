...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    //include_once(ROOT_PATH . 'includes/cls_ueditor.php'); // 包含 html editor 类文件
    ...
    //$ueditor = new ueditor('goods_desc');
    //$goods_desc_html = $ueditor->create_html_editor($goods['goods_desc']);
    //$smarty->assign('ueditor',$goods_desc_html);
...
//-- 编辑商品详细信息ueditor
/*------------------------------------------------------ */
elseif($_REQUEST['act'] == 'ueditor_goods_desc'){
    $goods_id = isset($_REQUEST['goods_id'])?intval($_REQUEST['goods_id']):0;
    if(!empty($goods_id)){
        include_once(ROOT_PATH . 'includes/cls_ueditor.php'); // 包含 html editor 类文件
        $goods = $GLOBALS['cidb']->select('goods_id,goods_desc')->where(array('goods_id'=>$goods_id))->get("goods")->row_array();
        $ueditor = new ueditor('goods_desc');
        $goods_desc_html = $ueditor->create_html_editor($goods['goods_desc']);
        $smarty->assign('goods_id',$goods['goods_id']);
        $smarty->assign('ueditor',$goods_desc_html);
        $smarty->assign('ueid','goods_desc');
        $smarty->display('goods/ueditor_goods_desc.html');
    }
}

elseif($_REQUEST['act'] == 'submit_ueditor_goods_desc'){
    $goods_desc = trim($_POST['desc']);
    $goods_id = isset($_REQUEST['goods_id'])?intval($_REQUEST['goods_id']):0;
    $result = array();
    $result['error'] = '';
    if(!empty($goods_id)){
        $sql = "UPDATE ".$GLOBALS['ecs']->table('goods')." SET goods_desc = '$goods_desc' WHERE goods_id=".$goods_id." LIMIT 1";
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    /*处理商品封面图*/
    if(!empty($_FILES['goods_index_img']['tmp_name'])){ 
        $goods_index_img = $image->upload_image($_FILES['goods_index_img']); 
        //删除原图片
        $sql = "SELECT goods_index_img " .
                    " FROM " . $ecs->table('goods') .
                    " WHERE goods_id = '$_REQUEST[goods_id]'";
        $row = $db->getRow($sql);
        if(!empty($row['goods_index_img']) && is_file(ROOT_PATH.'/'.$row['goods_index_img'])){
            @unlink(ROOT_PATH.'/'.$row['goods_index_img']);
        }
    }
    ...
    $goods_index_img = (!empty($goods_index_img) && isset($_FILES['goods_index_img'])) ? $goods_index_img :'';
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id,goods_index_img)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id', '$goods_index_img')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral,goods_index_img)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral', '$goods_index_img')";
        }
    }
    else
    {
        ...
        if($goods_index_img){
            $sql .= "goods_index_img = '$goods_index_img', ";
        }
        if ($code != '')
        {
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['type']))
    {
        ...
        /*转移到扩展分类*/
        elseif($_POST['type'] == 'move_to_extend_cat'){
            $cat_id = isset($_POST['target_extend_cat'])?intval($_POST['target_extend_cat']):0;
            $goods_id = explode(',', $goods_id);
            if(!empty($goods_id)){
                foreach($goods_id as $id){
                    //查找是否已经添加了扩展
                    $res = $GLOBALS['cidb']->where(array('goods_id'=>$id,'cat_id'=>$cat_id))->get('goods_cat')->row_array();
                    if(empty($res)){
                        //插入商品扩展分类数据
                        $insert = array();
                        $insert['goods_id'] = $id;
                        $insert['cat_id'] = $cat_id;
                        $GLOBALS['cidb']->insert('goods_cat',$insert);
                    }
                }
            }
        }
        /*取消扩展分类*/
        elseif($_POST['type'] == 'move_out_extend_cat'){
            $cat_id = isset($_POST['target_out_extend_cat'])?intval($_POST['target_out_extend_cat']):0;
            $goods_id = explode(',', $goods_id);
            if(!empty($goods_id)){
                foreach($goods_id as $id){
                    //查找是否已经添加了扩展
                    ...
                    if(!empty($res)){
                        //删除相关记录
                        $GLOBALS['cidb']->where(array('goods_id'=>$id, 'cat_id'=>$cat_id))->limit(1)->delete("goods_cat");                       
                    }
                }
            }
        }
        /*关联商品*/
        elseif($_POST['type'] == 'relate_goods'){
            $goods_id = !empty($_POST['checkboxes'])?$_POST['checkboxes']:array();
            //选中2个商品以上
            if(count($goods_id) > 1){
                $tmp_goods_id = $goods_id;
                foreach($goods_id as $key => $gid){
                    if(!isset($goods_id[$key+1])){
                        break;
                    }
                    else {
                        for($inner_key = $key+1;$inner_key < count($tmp_goods_id);$inner_key++){
                            $insert = array();
                            $insert['goods_id'] = $gid;
                            $insert['link_goods_id'] = $tmp_goods_id[$inner_key];
                            $insert['is_double'] = 1;
                            $insert['admin_id'] = $_SESSION['admin_id'];
                            //查询是否已经设置了双向关联
                            $sql = " SELECT COUNT(*) FROM ".$ecs->table('link_goods')." WHERE goods_id = ".$insert['goods_id']. " AND link_goods_id = ".$insert['link_goods_id']." AND is_double = 1";
                            $res = $db->getOne($sql);
                            if(empty($res)){
                               $GLOBALS['cidb']->insert("link_goods",$insert);
                            }
                            $insert1 = $insert;
                            $insert1['goods_id'] = $tmp_goods_id[$inner_key];
                            $insert1['link_goods_id'] = $gid;
                            $sql = " SELECT COUNT(*) FROM ".$ecs->table('link_goods')." WHERE goods_id = ".$insert1['goods_id']." AND link_goods_id = ".$insert1['link_goods_id']." AND is_double = 1";
                            $res1 = $db->getOne($sql);
                            if(empty($res1)){
                                $GLOBALS['cidb']->insert("link_goods",$insert1);
                            }
                        }
                    }
                }
        ...
        /*复制商品*/
        elseif($_POST['type'] == 'copy_goods'){
            $sql = "SELECT goods_id, goods_name, cat_id, brand_id FROM ".$ecs->table('goods')." WHERE goods_id IN(".$goods_id.')';
            $res = $db->query($sql);
            while( $row = $db->fetchRow($res)){
                //插入复制的商品
                $insert_sql = " INSERT INTO ".$ecs->table('goods')." (goods_name, cat_id, is_on_sale, goods_number, brand_id) VALUES ('{$row['goods_name']}', '{$row['cat_id']}', 0, 999, '{$row['brand_id']}')";
                $insert_res = $db->query($insert_sql);
                $insert_id = $db->insert_id($insert_res);             
                if($insert_id){
                    //更新货号
                    update_goods($insert_id, 'goods_sn', generate_goods_sn($insert_id));
                    $sql = "SELECT cat_id FROM ".$ecs->table('goods_cat')." WHERE goods_id = ".$row['goods_id'];
                    $res1 = $db->query($sql);
                    while($ext = $db->fetchRow($res1)){
                        $insert_ext = "INSERT INTO ".$ecs->table('goods_cat')." (goods_id, cat_id) VALUES ('$insert_id', '{$ext['cat_id']}')";
                        $db->query($insert_ext);
                    }
                }
            }
...
//-- 修改商品同类ID
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_goods_same_id')
{
    check_authz_json('goods_manage');

    $goods_id   = intval($_POST['id']);
    $goods_same_id = json_str_iconv(trim($_POST['val']));

    if ($exc->edit("goods_same_id = '$goods_same_id', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result(stripslashes($goods_same_id));
    }
}
...
//-- 修改商品简称
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_goods_short_name')
{
    check_authz_json('goods_manage');

    $goods_id   = intval($_POST['id']);
    $goods_short_name = json_str_iconv(trim($_POST['val']));

    if ($exc->edit("goods_short_name = '$goods_short_name', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result(stripslashes($goods_short_name));
    }
}
...
//-- 修改一听试购状态
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_tryonebuy')
{
    check_authz_json('goods_manage');

    $goods_id       = intval($_POST['id']);
    $is_tryonebuy         = intval($_POST['val']);

    if ($exc->edit("is_tryonebuy = '$is_tryonebuy', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result($is_tryonebuy);
    }
}
...
//-- 修改买七送一状态
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_buy_seven')
{
    check_authz_json('goods_manage');

    $goods_id       = intval($_POST['id']);
    $is_buy_seven   = intval($_POST['val']);

    if ($exc->edit("is_buy_seven = '$is_buy_seven', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result($is_buy_seven);
    }
}
...
//-- 修改买九送二状态
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_buy_nine')
{
    check_authz_json('goods_manage');

    $goods_id       = intval($_POST['id']);
    $is_buy_nine   = intval($_POST['val']);

    if ($exc->edit("is_buy_nine = '$is_buy_nine', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result($is_buy_nine);
    }
}
...
//-- 修改买五送一状态
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_buy_five')
{
    check_authz_json('goods_manage');

    $goods_id       = intval($_POST['id']);
    $is_buy_five    = intval($_POST['val']);

    if ($exc->edit("is_buy_five = '$is_buy_five', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result($is_buy_five);
    }
}
...
//-- 修改买六送一状态
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_buy_six')
{
    check_authz_json('goods_manage');

    $goods_id       = intval($_POST['id']);
    $is_buy_six    = intval($_POST['val']);

    if ($exc->edit("is_buy_six = '$is_buy_six', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result($is_buy_six);
    }
}
...
//-- 修改共四罐状态
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_total_four')
{
    check_authz_json('goods_manage');

    $goods_id       = intval($_POST['id']);
    $is_total_four   = intval($_POST['val']);

    if ($exc->edit("is_total_four = '$is_total_four', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result($is_total_four);
    }
}
...
//-- 修改共两罐状态
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_total_two'){
    check_authz_json('goods_manage');
    
    $goods_id       =  intval($_POST['id']);
    $is_total_two   =  intval($_POST['val']);
    
    if($exc->edit("is_total_two = '$is_total_two', last_update=" .  gmtime(), $goods_id)){
        clear_cache_files();
        make_json_result($is_total_two);
    }
}
