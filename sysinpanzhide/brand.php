...
elseif ($_REQUEST['act'] == 'edit')
{
    /* 权限判断 */
    admin_priv('brand_manage');
    $sql = "SELECT brand_id, brand_name, site_url, brand_logo, brand_desc, brand_logo, is_show, sort_order, brand_descImg ".
...
elseif ($_REQUEST['act'] == 'updata')
{
    ...
    /* 处理图片 */
    $img_name = basename($image->upload_image($_FILES['brand_logo'],'brandlogo'));
    $desc_img = $image->upload_image($_FILES['brand_descImg'],'branddescImg');
...
//-- 切换是否买五送一
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_preferential_five')
{
    check_authz_json('brand_manage');
    ...
    $exc->edit("is_preferential_five='$val'", $id);
...
//-- 切换是否买七送一
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_preferential_seven')
{
    check_authz_json('brand_manage');
    ...
    $exc->edit("is_preferential_seven='$val'", $id);
...
function get_brandlist()
{
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('goods')
             . ' WHERE `brand_id` = ' . $rows['brand_id']
             . ' AND `is_delete` = 0';
        FB::log($sql);
        $rows['count'] = $GLOBALS['db']->getOne($sql);
