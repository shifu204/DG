 ...
 * @return      void
 */
function sys_msg($msg_detail, $msg_type = 0, $links = array(), $auto_redirect = true, $seconds = 3)
{
    if (count($links) == 0)
    {
    ...
    $GLOBALS['smarty']->assign('seconds', $seconds);
...
// 获得所会会员的有效历史订单数量
function getCountHistoryOrder() {
    // 统计用户的历史订单数（包括已付款、已发货（未付款）、已收货）
    $sql = 'SELECT `user_id`, count(`user_id`) AS count FROM ' . $GLOBALS['ecs']->table('order_info')
         . ' WHERE `pay_status` = ' . PS_PAYED . ' OR `shipping_status` in (' . SS_SHIPPED . ',' . SS_RECEIVED . ') GROUP BY `user_id`';
    $res = $GLOBALS['db']->query($sql);
    while ($row_order = $GLOBALS['db']->fetchRow($res)) {
        $count_history_order[$row_order['user_id']] = $row_order['count'];
    }
    return $count_history_order;
}
?>
