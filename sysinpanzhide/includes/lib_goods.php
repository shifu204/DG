...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
	...
	$tmp = $GLOBALS['db']->getRow('Describe ' . $GLOBALS['ecs']->table('goods') . ' is_tryonebuy');
	if(!$tmp){
		$GLOBALS['db']->query('ALTER TABLE ' . $GLOBALS['ecs']->table('goods') . ' '.
		'ADD COLUMN `is_tryonebuy`  tinyint(1) UNSIGNED NULL DEFAULT 0 AFTER `is_check`'
		);
	}
    /* 过滤条件 */
    ...
    if ($result === false)
    {
        ...
        $from = "SELECT gc.goods_id, gc.cat_id, g.goods_same_id, g.goods_short_name, g.goods_name, g.goods_type, g.goods_sn, g.shop_price, g.is_on_sale, g.is_best, g.is_new, g.is_hot, g.is_tryonebuy, g.is_buy_six, g.is_buy_nine, g.is_total_four, g.is_total_two, g.sort_order, g.goods_number, g.integral, g.promote_price, g.promote_start_date, g.promote_end_date, g.is_delete, g.is_real, g.brand_id, g.suppliers_id, g.goods_sub_title ".
                " FROM ecs_goods_cat as gc LEFT JOIN ecs_goods as g ON g.goods_id = gc.goods_id".
                " union all ".
                "SELECT goods_id, cat_id, goods_same_id, goods_short_name, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, is_tryonebuy, is_buy_six, is_buy_nine, is_total_four, is_total_two, sort_order, goods_number, integral, promote_price, promote_start_date, promote_end_date, is_delete, is_real, brand_id, suppliers_id, goods_sub_title".
                "  FROM ".$GLOBALS['ecs']->table('goods');
        /* 记录总数 */
        $sql = "SELECT COUNT(DISTINCT goods_id ) FROM (" .$from.  ") AS g WHERE is_delete='$is_delete' $where";
        $filter['record_count'] =$GLOBALS['db']->getOne($sql);

        /* 分页大小 */
        $filter = page_and_size($filter);        

        $sql = "SELECT goods_id, goods_same_id, goods_short_name, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, is_tryonebuy, is_buy_six, is_buy_nine, is_total_four, is_total_two, sort_order, goods_number, integral, goods_sub_title," .
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
                    " FROM (" . $from . ") AS g WHERE is_delete='$is_delete' $where" .
                    " GROUP BY goods_id".
