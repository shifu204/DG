...
if ($_REQUEST['act'] == 'list')
{
    ...
    assign_query_info();
    if(isset($_SESSION['comment_manage']['filter'])){
        $smarty->assign('filter', $_SESSION['comment_manage']['filter']);
    }
...
//-- 返还金额，点数
/*------------------------------------------------------ */
if($_REQUEST['act'] == 'return_amount_point'){
    $return_amount = isset($_REQUEST['return_amount'])?floatval($_REQUEST['return_amount']):0;
    $return_integral = isset($_REQUEST['return_integral'])?floatval($_REQUEST['return_integral']):0;
    ...
    if(!empty($comment_id)){
        ...
        if(!empty($comment_info) && $comment_info['is_return'] == 0){
            $sql = "UPDATE ".$ecs->table('comment')." SET return_amount = $return_amount, return_integral = $return_integral, is_return = 1 WHERE comment_id = $comment_id";
            $db->query($sql);
            if($return_amount || $return_integral){
                log_account_change($comment_info['user_id'], $return_amount, 0, 0, $return_integral, '晒单奖励' , ACT_SHAIDAN);
            }
            sys_msg('已成功返还金额点数', 0, array(array('text'=>'返回评论列表','href'=>'comment_manage.php?act=list')));
        } else {
            sys_msg('该评论已返还', 0, array());
        }
    }
}
/*------------------------------------------------------ */
...
if ($_REQUEST['act']=='reply')
{
    ...
    $comment_info['user_id'] = $comment_info['user_id'];
    ...
    if(!empty($comment_info['token'])){
        $sql = "SELECT * FROM ".$ecs->table('shaidan_img')." WHERE token = '".$comment_info['token']."'";
        $imgs = $db->getAll($sql);
        $comment_info['imgs'] = $imgs;
    }
    /* 获取管理员的用户名和Email地址 */
...
//-- 是否推荐评论到用户中心
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_is_recommend'){
    $value = intval($_REQUEST['val']);
    $comment_id = intval($_REQUEST['id']);
    $sql = "UPDATE " . $GLOBALS['ecs']->table("comment") . " SET is_recommend = " . $value . " WHERE comment_id = " . $comment_id;
...
function get_comment_list()
{
    ...
    if (isset($_REQUEST['is_ajax']) && $_REQUEST['is_ajax'] == 1)
    {
        ...
        $filter['username']     = empty($_REQUEST['username']) ? 0 : trim($_REQUEST['username']);
        $filter['status']     = !isset($_REQUEST['status']) ? -1 : intval($_REQUEST['status']);
        $filter['comment_type'] = isset($_REQUEST['comment_type']) ? ($_REQUEST['comment_type'] == -1 ? -1 : $_REQUEST['comment_type']) : -1;       
        $filter['keywords'] = json_str_iconv($filter['keywords']);
        $filter['username'] = json_str_iconv($filter['username']);
    } else if(isset($_SESSION['comment_manage']['filter'])){
        $filter = $_SESSION['comment_manage']['filter'];
    }
    ...
    if(!empty($filter['username'])){
        $where .= " AND user_name LIKE '%" . mysql_like_quote($filter['username']) . "%'";
    }
    if($filter['status'] != -1 && isset($filter['status'])){
        $where .= " AND status = " . $filter['status'];
    }
    
    if($filter['comment_type'] != -1 && isset($filter['comment_type'])){
        $where .= " AND comment_type = " . $filter['comment_type'];
    }
    ...
    /* 获取评论数据 */
    $arr = array();
    $sql  = "SELECT * FROM " .$GLOBALS['ecs']->table('comment'). " WHERE user_id > 0 $where " .
...
//            $GLOBALS['_LANG']['yes_reply'] : $GLOBALS['_LANG']['no_reply'];
    ...
    /*保存搜索条件*/
    $_SESSION['comment_manage']['filter'] = $filter;
    return $arr;
}
