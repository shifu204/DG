...
if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    if (!$isadd)
    {
        ...
        //获取专题商品表
        $sql = "SELECT g.goods_name, g.shop_price, tg.* FROM " . $GLOBALS['ecs']->table('topic_goods') . " AS tg " . 
        ...
        foreach($goods_list as $gk=>$goods){
            if(empty($goods['topic_goods_name'])){
                $goods_list[$gk]['topic_goods_name'] = '修改';
            }
            if(empty($goods['tag1'])){
                $goods_list[$gk]['tag1'] = '修改';
            }
        }
        ...
        array_unshift($class_list,array('class_id'=>0,'class_name'=>$GLOBALS['_LANG']['default_class']));
        ...
        if(!empty($class_list)){
            foreach($class_list as $class){
                $sql = "SELECT g.goods_name, g.shop_price, tg.* FROM " . $GLOBALS['ecs']->table('topic_goods') . " AS tg " . 
                       " LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " AS g ON g.goods_id = tg.goods_id " . 
                       " WHERE tg.topic_id = $topic_id AND tg.class_id = {$class['class_id']}";
                $goods = $GLOBALS['db']->getAll($sql);
                if(!empty($goods)){
                    foreach($goods as $g){
                        $class_goods[$class['class_id']][$g['goods_id']] = $g;
                    }
                    
                } else {
                    $class_goods[$class['class_id']] = array();
                }
            }
        }
        $smarty->assign('class_goods', $class_goods);
        $smarty->assign('myTopic', json_encode($class_goods));
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_insert)
    {
        $sql = "INSERT INTO " . $ecs->table('topic') . " (title,start_time,end_time,intro,template,css,topic_img,title_pic,base_style, htmls,keywords,description)" .
                "VALUES ('$_POST[topic_name]','$start_time','$end_time','$_POST[topic_intro]','$_POST[topic_template_file]','$_POST[topic_css]', '$topic_img', '$title_pic', '$base_style', '$htmls','$keywords','$description')";
    }
    else
    {
        $sql = "UPDATE " . $ecs->table('topic') .
                "SET title='$_POST[topic_name]',start_time='$start_time',end_time='$end_time',intro='$_POST[topic_intro]',template='$_POST[topic_template_file]',css='$_POST[topic_css]', topic_img='$topic_img', title_pic='$title_pic', base_style='$base_style', htmls='$htmls', keywords='$keywords', description='$description'" .
...
elseif ($_REQUEST['act'] == 'get_goods_list')
{
    ...
    $_REQUEST['cat_id'] = $filters->cat_id;
    $_REQUEST['brand_id'] = $filters->brand_id;
    $_REQUEST['keyword'] = $filters->keyword;
    $_REQUEST['page_size'] = 2000;
    $arr = goods_list(0);
    $arr = $arr['goods'];
    $opt = array();
...
elseif($_REQUEST['act'] == 'goods_sort_order'){
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json          = new JSON;
    $data = $json->decode($_REQUEST['JSON']);
    $goods_id = $data->goods_id;
    $sort_order = $data->val;
    $topic_id = $data->topic_id;
    if($sort_order < 0 ){
        make_json_error($_LANG['sort_order_error']);
    }
    $db->autoExecute($GLOBALS['ecs']->table('topic_goods'), array('sort_order'=>$sort_order), 'UPDATE', "goods_id = $goods_id AND topic_id = $topic_id");
    make_json_result(stripslashes($sort_order));
}
elseif($_REQUEST['act'] == 'goods_topic_price'){
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json          = new JSON;
    $data = $json->decode($_REQUEST['JSON']);
    $goods_id = $data->goods_id;  
    ...
    $price = $data->val;
    if($price < 0 ){
        make_json_error($_LANG['price_lt_0']);
    }
    $db->autoExecute($GLOBALS['ecs']->table('topic_goods'), array('price'=>$price), 'UPDATE', "goods_id = $goods_id AND topic_id = $topic_id");
    make_json_result(stripslashes($price));
}
elseif($_REQUEST['act'] == 'remove_topic_goods'){
    $post_id = trim($_GET['id']);
    $arr = explode('|', $post_id);
    $goods_id = $arr[0];
    $topic_id = $arr[1];
    $sql = " DELETE FROM " . $GLOBALS['ecs']->table('topic_goods') . " WHERE topic_id = $topic_id AND goods_id = $goods_id";
    $GLOBALS['db']->query($sql);
}
elseif($_REQUEST['act'] == 'goods_index_img'){
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json          = new JSON;
    $data = $json->decode($_REQUEST['JSON']);
    $goods_id = $data->goods_id;   
    ...
    $img = $data->val;
    if(empty($img)){
        make_json_error($_LANG['field_empty']);
    }
    $db->autoExecute($GLOBALS['ecs']->table('topic_goods'), array('index_img'=>$img), 'UPDATE', "goods_id = $goods_id AND topic_id = $topic_id");
    make_json_result("<img src='". $img ."' width='75'>");
}
elseif($_REQUEST['act'] == 'goods_topic_name'){
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json          = new JSON;
    $data = $json->decode($_REQUEST['JSON']);
    $goods_id = $data->goods_id;   
    $topic_id = $data->topic_id;
    $topic_goods_name = $data->val;
    if(empty($topic_goods_name)){
        make_json_error($_LANG['field_empty']);
    }
    $db->autoExecute($GLOBALS['ecs']->table('topic_goods'), array('topic_goods_name'=>$topic_goods_name), 'UPDATE', "goods_id = $goods_id AND topic_id = $topic_id");
    make_json_result($topic_goods_name);
}
elseif($_REQUEST['act'] == 'goods_tag1'){
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json          = new JSON;
    $data = $json->decode($_REQUEST['JSON']);
    $goods_id = $data->goods_id;   
    ...
    $db->autoExecute($GLOBALS['ecs']->table('topic_goods'), array('tag1'=>$goods_tag1), 'UPDATE', "goods_id = $goods_id AND topic_id = $topic_id");
    make_json_result($goods_tag1);
}
elseif($_REQUEST['act'] == 'add_topic_class'){
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json          = new JSON;
    $data = $json->decode($_REQUEST['JSON']);
    $topic_id = $data->topic_id;
    ...
    $insert = array();
    $insert['topic_id'] = $topic_id;
    $insert['class_name'] = $class_name;
    $insert['sort_order'] = 0 ;
    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('topic_class'),$insert);
    make_json_result($GLOBALS['db']->insert_id());
}
elseif($_REQUEST['act'] == 'delete_topic_class'){
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json          = new JSON;
    $data = $json->decode($_REQUEST['JSON']);
    $topic_id = $data->topic_id;
    ...
    //删除分类
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('topic_class') . " WHERE topic_id = $topic_id AND class_id = $class_id LIMIT 1";
    $GLOBALS['db']->query($sql);
    //删除分类下的商品
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('topic_goods') . " WHERE topic_id = $topic_id AND class_id = $class_id ";
    $GLOBALS['db']->query($sql);
    make_json_result(stripslashes('1'));
}
elseif($_REQUEST['act'] == 'add_goods'){
    ...
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('topic_goods') . " WHERE topic_id = $topic_id AND class_id = $class_id AND goods_id = $goods_id";
    $res = $GLOBALS['db']->getRow($sql);
    if(empty($res)){
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('topic_goods'),$insert);
        make_json_result(stripslashes('0'));
    }
    else {
        make_json_result(stripslashes('1'));
    }
...
elseif($_REQUEST['act'] == 'delete_goods'){
    ...
    $class_id = $data->class_id;
    $goods_id = $data->goods_id;
    $where = " topic_id = $topic_id AND class_id = $class_id ";
    if($goods_id){
        $where .=" AND goods_id = $goods_id";
    }
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('topic_goods') . " WHERE $where";
    $GLOBALS['db']->query($sql);
    make_json_result(stripslashes('1'));
}
/**
