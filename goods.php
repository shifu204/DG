...
$is_tryone = isset($_REQUEST['is_tryone'])  ? intval($_REQUEST['is_tryone']) : 0;
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'price')
{
    ...
    else
    {
        ...
        $res['ori_shop_price'] = $shop_price * $number;
        $res['round_shop_price'] = round($shop_price * $number);
    }
    die($json->encode($res));
}
...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
        ...
        $smarty->assign('goods_same_id',      $goods['goods_same_id']);
        $smarty->assign('goods_id',           $goods['goods_id']);
        $smarty->assign('promote_end_time',   $goods['gmt_end_time']);
        $smarty->assign('categories',         get_categories_tree($goods['cat_id']));  // 分类树
        if(!empty($goods['goods_same_id'])){
            $smarty->assign('comment_num', get_goods_comment_num($goods['goods_same_id'], '0,1'));  //评论数
        } else {
            $smarty->assign('comment_num', get_goods_comment_num($goods['goods_id'], '0,1'));  //评论数
        }      
        /* meta */
        ...
        $catlist = array();
        $all_cats = array(); //该商品所处分类下的分类列表
        //所有顶级分类
        $tmp = get_parent_cats($goods['cat_id']);
        //顶级分类的个数
        $tmp_count = count($tmp);
        foreach($tmp as $k=>$v)
        {
            ...
            //只需要获取最顶级分类的分类树
            if($k == $tmp_count - 1){
                $all_cats = array_keys(cat_list($v['cat_id'],0,false));
            }
        }
        $all_cats_str = implode(',', $all_cats);
        //获取其他客户购买的记录
        if(!empty($all_cats)){
            $sql_child = "SELECT og.rec_id, g.goods_id, g.goods_name,g.goods_img, g.goods_same_id FROM ".$GLOBALS['ecs']->table("order_goods"). " AS og ".
                          'LEFT JOIN '.$GLOBALS['ecs']->table("goods").' AS g  ON og.goods_id = g.goods_id '.
                          "WHERE g.cat_id IN ($all_cats_str)".' ORDER BY og.rec_id DESC';
            $res = $GLOBALS['db']->getALL($sql_child);
            $sql_others = "SELECT * FROM ( ".$sql_child ." ) as result GROUP BY goods_id ORDER BY  rec_id DESC LIMIT 8";
            $others = $GLOBALS['db']->getALL($sql_others);
            if(!empty($others)){
                foreach($others as $okey => $other){
                    if(empty($other['goods_img'])){
                        if(!empty($other['goods_same_id'])){
                            $sql = "SELECT goods_thumb, goods_img, original_img FROM ".$GLOBALS['ecs']->table('goods')." WHERE goods_id = {$other['goods_same_id']}";
                            $same_goods = $GLOBALS['db']->getRow($sql);
                            $others[$okey]['goods_img'] = $same_goods['goods_img'];
                            $others[$okey]['goods_thumb'] = $same_goods['goods_thumb'];
                            $others[$okey]['original_img'] = $same_goods['original_img'];
                        }
                    }
                }
            }
        ...
        if(!empty($properties['pro'])){
            if(isset($properties['pro']['商品属性'])){
                $pro_count = 1;
                foreach($properties['pro']['商品属性'] as $pro){
                    if($pro_count <= 3){
                        $show_property[] = $pro;
                    }else {
                        break;
        ...
        $smarty->assign('show_property',$show_property);
        ...
        $pictures = get_goods_gallery($goods_id);                                                // 商品相册
        if(empty($pictures) && !empty($goods['goods_same_id'])){
            $pictures = get_goods_gallery($goods['goods_same_id']);
        }
        ...
        //商品详情页顶部广告
        $ads = get_ads_by_tag('goods_detail_top');
...
$smarty->assign('is_tryone',  $is_tryone);           // 当前系统时间
$current_tab  = $_GET['tab'] ? intval($_GET['tab']) : 0;

$smarty->assign('current_tab',  $current_tab);           // 只显示说明
...
function get_linked_goods($goods_id)
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                ...
                'g.market_price, g.promote_price, g.promote_start_date, g.promote_end_date, g.goods_same_id ' .
            ...
            "WHERE lg.goods_id = '$goods_id' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ORDER BY g.sort_order ASC ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if(empty($row['goods_thumb'])){
            ...
            if(!empty($row['goods_same_id'])){
                $sql = "SELECT goods_thumb, goods_img, original_img FROM ".$GLOBALS['ecs']->table('goods')." WHERE goods_id = ".$row['goods_same_id'];
                $same_goods = $GLOBALS['db']->getRow($sql);
                $arr[$row['goods_id']]['goods_thumb'] = $same_goods['goods_thumb'];
                $arr[$row['goods_id']]['goods_img'] = $same_goods['goods_img'];
                $arr[$row['goods_id']]['original_img'] = $same_goods['original_img'];
            }
        }
    }
    return $arr;
