...
(function () {
    ...
    OnlineImage.prototype = {
        ...
        initEvents: function () {
            ...
            domUtils.on(this.container, 'click', function (e) {
                ...
                if (li.tagName.toLowerCase() == 'li') {
                    if(typeof(li.attributes['is_dir']) != 'undefined'){
                        if(li.attributes['is_dir'].value == 1){
                            _this.filedir =li.attributes['filedir'].value;
                            _this.listEnd = false;
                        ...
                        li.removeAttribute("selectedNum");
                    } else {
                        selectedImageNum ++;
                        domUtils.addClass(li, 'selected');
                        li.setAttribute("selectedNum",selectedImageNum);
                    }
                }
            });
        },
        ...
        getImageData: function () {            
            var _this = this;
            if(typeof(_this.filedir) == 'undefined'){
                _this.filedir = '';
            }
        ...
        pushData: function (list) {
            var i, item, img, icon,name, _this = this,
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    name = document.createElement('font');
                    ...
                    name.innerHTML = list[i].filename ;
                    domUtils.addClass(name,'filename');
                    if(list[i].is_dir == 1){
                        item.setAttribute('is_dir',1);
                        item.setAttribute('filedir',list[i].url);
                        img = document.createElement('div');
                        domUtils.addClass(img,'dir');
                    }
            ...
            for (i = 0; i < lis.length; i++) {
                if (domUtils.hasClass(lis[i], 'selected')) {
                    var img = lis[i].firstChild,
                        ...
                        /*点击图片的循序*/
                        num = lis[i].attributes.selectedNum.value;                       
                    list.push({
                        src: src,
                        _src: src,
