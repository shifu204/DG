...
$page = 1;
if(isset($_REQUEST['page']) && isset($_REQUEST['_p'])){
    $tmp = explode("#", $_REQUEST['page']);
...
$nav_filters = $GLOBALS['cidb']->select('cat_id,cat_name')->where('cat_tag = "'.$tag_name.'" AND is_show = 0 ')->order_by('sort_order ASC')->get('category')->result_array();
...
if($_REQUEST['act'] == 'ajax') {
    ...
    $pager = get_ajax_pager('getData', array(), $total, $page, $page_size,"#content_area");
...
$smarty->assign('total',$reutnr['total']);
$smarty->assign('filt',$filt);
$pager = get_ajax_pager('getData', array(), $return['total'], $page, $page_size,"#content_area");
$smarty->assign('pager',$pager);
...
function get_goods($filt = array(),$page_size = 24 , $offset = 0){
    ...
    $GLOBALS['cidb']->select("goods.*")->where_in("goods.cat_id",$children)->where(array("goods.is_on_sale"=>1,"goods.is_delete"=>0))->order_by("goods.sort_order ASC");

    //如果选中品牌
    if(isset($filt['brand_id']) && $filt['brand_id'] > 0){
        $GLOBALS['cidb']->where(array("goods.brand_id"=>$filt['brand_id']));
    }
    ...
    $goods_sql =unserialize(serialize($GLOBALS['cidb']));
    $total = $goods_sql->get()->num_rows();  
    $goods = $GLOBALS['cidb']->limit($page_size,$offset)->get()->result_array();
    //对商品信息进行格式化
