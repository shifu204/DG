...
function reg(str){
  var bt=getEid(str+"_b").getElementsByTagName("h2");
  for(var i=0;i<bt.length;i++){
    bt[i].subj=str;
    bt[i].pai=i;
    bt[i].style.cursor="pointer";
    bt[i].onclick=function(){
      getEid(this.subj+"_v").innerHTML=getEid(this.subj+"_h").getElementsByTagName("blockquote")[this.pai].innerHTML;
      for(var j=0;j<getEid(this.subj+"_b").getElementsByTagName("h2").length;j++){
        var _bt=getEid(this.subj+"_b").getElementsByTagName("h2")[j];
        var ison=j==this.pai;
        _bt.className=(ison?"":"h2bg");
      }
    }
  }
  getEid(str+"_h").className="none";
  getEid(str+"_v").innerHTML=getEid(str+"_h").getElementsByTagName("blockquote")[0].innerHTML;
}
...
function mypicBg(){
    var img = $("#imglist li img");
    $.each(img,function(){
        $(this).click(function(){
            $(img).removeClass("onbg");
            $(this).addClass("onbg");
        });
    });
}
