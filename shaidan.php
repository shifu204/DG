...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'add_pinglun')
{
	$user_id = $_SESSION['user_id'];
	$msg_id = intval($_POST['s_id']);
	$goods_id = intval($_POST['goods_id']);
	//_P($_POST,1);
    $sql = 'SELECT rec_id FROM ' . $GLOBALS['ecs']->table('shaidan')
         . ' WHERE msg_id = ' . $msg_id;
    $rec_id = $GLOBALS['db']->getOne($sql);
	if(!$user_id || !$msg_id || !$_POST){
		show_message('暂不开放晒单回复评论! 请稍后再试...', '返回晒单专区', 'shaidan_list.php', 'error');
	}
    ...
    $message = array(
        ...
        'msg_title'   => '',
        'goods_id'   => $goods_id,
        'msg_content' => isset($_POST['p_content']) ? trim($_POST['p_content']) : '',
        'order_id'=>empty($_POST['order_id']) ? 0 : intval($_POST['order_id']),
        'upload'      => array()
     );

    if (add_shaidan($message, true))
    {
        show_message('回复成功，等待审核通过即可显示', '返回晒单详细','shaidan.php?id='.$goods_id.'&s_id='.$msg_id,'info');
    }
    else
    {
        show_message('回复失败，请重新尝试发表回复', '返回晒单详细', 'shaidan.php?id='.$goods_id.'&s_id='.$msg_id,'info');
    }
...
$cache_id = $goods_id . '-' . $s_id . '-' . $_SESSION['user_rank'].'-'.$_CFG['lang'];
$cache_id = sprintf('%X', crc32($cache_id));//_P($smarty->is_cached('shaidan.dwt', $cache_id),1);
if (!$smarty->is_cached('shaidan.dwt', $cache_id))
{
...
$sql = 'SELECT shaidan_img FROM ' . $GLOBALS['ecs']->table('shaidan_img')
     . ' WHERE token = "' . $shaidan['token'] . '"';
$images = $GLOBALS['db']->getAll($sql, 1);
$shaidan['images'] = $images;
FB::info($shaidan);
...
if($shaidan){
	$sql   = "SELECT * FROM " .$GLOBALS['ecs']->table('shaidan') .
			 " WHERE msg_status='1' and parent_id = '" . $shaidan['msg_id'] . "'";
	$shaidanAll = $GLOBALS['db']->getAll($sql);
}
//_P($shaidanAll,1);

$smarty->assign('shaidanAll',  $shaidanAll);
$smarty->display('shaidan.dwt');
